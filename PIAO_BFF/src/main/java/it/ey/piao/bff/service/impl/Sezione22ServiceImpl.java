...
public class Sezione22ServiceImpl implements ISezione22Service {
    ...
    @Override
    public Mono<GenericResponseDTO<Sezione22DTO>> saveOrUpdate(Sezione22DTO request) {
        log.info("Richiesta salvataggio/modifica Sezione22");
        HttpHeaders headers = new HttpHeaders();
        headers.set("Accept", "application/json");
        return webClientService.post("/sezione22/save", webServiceType, request, headers, Sezione22DTO.class)
            .doOnNext(response -> log.info("Sezione22 Salvata/Modificata: {}", response))
            .map(sezione22 -> {
