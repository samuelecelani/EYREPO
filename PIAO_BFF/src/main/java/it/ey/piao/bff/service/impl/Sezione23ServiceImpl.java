...
public class Sezione23ServiceImpl implements ISezione23Service {
    ...
    @Override
    public Mono<GenericResponseDTO<Sezione23DTO>> saveOrUpdate(Sezione23DTO request) {
        log.info("Richiesta salvataggio/modifica Sezione23");
        HttpHeaders headers = new HttpHeaders();
        headers.set("Accept", "application/json");
        return webClientService.post("/sezione23/save", webServiceType, request, headers, Sezione23DTO.class)
            .doOnNext(response -> log.info("Sezione23 Salvata/Modificata: {}", response))
            .map(sezione23 -> {
