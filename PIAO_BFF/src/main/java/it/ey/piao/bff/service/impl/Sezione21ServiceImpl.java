...
public class Sezione21ServiceImpl implements ISezione21Service {
    ...
    public Mono<GenericResponseDTO<Sezione21DTO>> saveOrUpdate(Sezione21DTO request) {
        ...
        return webClientService.post("/sezione21/save",webServiceType,request,headers,Sezione21DTO.class)
            .doOnNext(response -> log.info("Sezione21 Salvata/Modficata: {}", response))
            .map(sezione21-> {
    ...
    public Mono<GenericResponseDTO<Sezione21DTO>> getOrCreate(PiaoDTO request) {
        ...
        HttpHeaders headers = new HttpHeaders();
        headers.set("Accept", "application/json");
        String url = "/sezione21/piao";
        return webClientService.post(url, webServiceType,request, headers, Sezione21DTO.class)
            ...
            .onErrorResume(e -> {
                ...
                GenericResponseDTO<Sezione21DTO> errorResponse = new GenericResponseDTO<>();
                errorResponse.setStatus(new Status());
                errorResponse.getStatus().setSuccess(Boolean.FALSE);
                errorResponse.setError(new Error());
                errorResponse.getError().setMessageError(e.getMessage());
