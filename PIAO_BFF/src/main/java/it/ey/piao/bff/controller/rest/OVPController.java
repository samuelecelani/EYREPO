...
public class OVPController {
    ...
    @GetMapping("/matrice/sezione21/{id}/sezione1/{id1}")
    public Mono<ResponseEntity<GenericResponseDTO<List<ItemMatriceDTO>>>> getMatrice(@PathVariable Long id,@PathVariable Long id1) {
        return ovpService.getOvpMatriceByIdSezione21(id,id1).map(ResponseEntity::ok);
    }
