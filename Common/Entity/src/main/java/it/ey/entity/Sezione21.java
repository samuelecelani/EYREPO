...
public class Sezione21 extends CampiTecnici {
    ...
    @OneToMany(mappedBy = "sezione21", cascade = CascadeType.ALL, orphanRemoval = false)
    private List<FondiEuropei> fondiEuropei;


    @OneToMany(mappedBy = "sezione21",orphanRemoval = false)
    @Filter(name = "activeFilter", condition = "X_ACTIVE = true")
    private List<OVP> ovpList;



    @OneToMany( mappedBy = "sezione21", cascade = { CascadeType.PERSIST, CascadeType.MERGE }, orphanRemoval = false)
    @Filter(name = "activeFilter", condition = "X_ACTIVE = true")
