...
import { PiaoPeriodoCompilazioneDirective } from '../../../directives/piao-periodo-compilazione.directive';
import { getValue } from '../../../config/loader-config';

@Component({
  selector: 'piao-modal-validazione',
  imports: [SharedModule, TextAreaComponent, CardAlertComponent, PiaoPeriodoCompilazioneDirective],
...
export class ModalValidazioneComponent extends ModalBodyComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.isOsservazioni) {
      this.deadline = getValue('dataScadenzaPiao');
