...
import { forkJoin, map, of, switchMap, takeUntil, tap } from 'rxjs';
...
import {
  cleanSingleMongoDTO,
  createFormIndicatoreFromPiaoSession,
...
export class IndicatoriComponent extends BaseComponent implements OnInit {
    ...
    if (formValue.choice === 'recupera') {
      ...
      const indicatoreRequest = {
        ...
        campiModificati: getChangedFields(
          ...
          'indicatori.' + this.codTipologiaIndicatoreFK
        ),
      };

      this.indicatoreService
        .saveIndicatore(indicatoreRequest, this.isAddNewIndicatore, this.sectionEnum)
        .pipe(takeUntil(this.destroy$)).subscribe({
          next: (data: any) => {
            if (this.isAddNewIndicatore) {
              ...
              // Ricostruiamo l'intero FormGroup 'indicatore' con la factory usata al load del PIAO.
              ...
              if (index !== -1) {
                const wrapperGroup = formArray.at(index) as FormGroup;
                const newIndicatoreFg = createFormIndicatoreFromPiaoSession(this.fb, data);
                if (newIndicatoreFg) {
                  wrapperGroup.setControl('indicatore', newIndicatoreFg);
                }
              }
            }
            // Chiudi il modal e resetta il form solo dopo che tutto è completato
  ...
  handleRelevant(element: IIndicatoreWrapper) {
    ...
    switch (this.codTipologiaFK) {
      case CodTipologiaIndicatoreEnum.OVP:
        this.indicatoreService.saveIndicatore(indicatoreData, false, this.sectionEnum).pipe(takeUntil(this.destroy$)).subscribe({
          next: () => {
          },
          error: (err) => {
  ...
  handleRemove(element: IIndicatoreWrapper) {
    if (!element.indicatore?.id) {
      // Se l'indicatore non ha ancora un id, significa che è locale, rimuovilo direttamente
    ...
    // Se ha un id, effettua la delete via API
    this.indicatoreService
      .deleteIndicatore(
        element.indicatore.id,
        this.piaoDTO.id,
        'indicatori.' + this.codTipologiaIndicatoreFK,
        this.testoSezione
      )
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: () => {
          // Delete riuscita, rimuovi dal FormArray locale
          const index = this.indicatori.controls.findIndex((control) => {
            const value = control.value as IIndicatoreWrapper;
            return value.indicatore?.id === element.indicatore.id;
          });
        ...
        error: (err) => {
          console.error("Errore nell'eliminazione dell'indicatore:", err);
        },
      });
  }
