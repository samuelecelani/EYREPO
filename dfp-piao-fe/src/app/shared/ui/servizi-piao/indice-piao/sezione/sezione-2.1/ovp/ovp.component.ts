...
export class OvpComponent extends BaseComponent implements OnInit {
  ...
  @ViewChild('strategieAttuative') strategieAttuativeComponent!: StrategieAttuativeComponent;
    ...
    this.modalService.onConfirmAction$
      ...
      .subscribe(({ metadato }) => {
        switch (metadato.value as string) {
          case CodDeleteImpactEnum.OVP:
            this.handleConfirmDeleteOvp({ id: metadato.idFK! }, true);
            break;
          case CodDeleteImpactEnum.STRATEGIA:
            this.strategieAttuativeComponent.handleRemoveStrategia({ id: metadato.idFK! }, true);
            break;
          default:
            break;
        }
  ...
  handleConfirmDeleteOvp(ovpToDelete: OVPDTO,forceDelete: boolean = false): void {
    if (this.isDettaglio) return;

    // Trova l'indice dell'elemento da eliminare
    const index = this.ovp.controls.findIndex(
      (control) => (control as FormGroup).controls['id'].value === ovpToDelete.id
    );
    ...
    if (ovpToDelete.id) {
      this.ovpService
        .delete(ovpToDelete.id, this.idPiao || -1, this.testoSezione, forceDelete)
    ...
    if (modalBody && modalBody.formGroup && modalBody.formGroup.valid) {
      ...
      const ovpRequest = {
        ...
        campiModificati: getChangedFields(
          ovpDTO,
          this.ovpDataArray[this.editingOvpIndex || -1],
          [
            'id',
            'ovpStrategias',
            'risorseFinanziarie',
      ...
      this.ovpService.save(ovpRequest).pipe(takeUntil(this.destroy$)).subscribe({
        next: (savedOvp) => {
          this.isSaving = false;
