...
import { Subscription, forkJoin, of, takeUntil } from 'rxjs';
  ...
  implements OnInit, OnDestroy, OnChanges
{
  ...
  ngOnInit(): void {
    ...
    this.subscription.add(
      this.sezione4Service.onSezione4Updated$.pipe(takeUntil(this.destroy$)).subscribe((sezione4: Sezione4DTO) => {
        if (sezione4) {
          this.loadMilestones();
        }
      })
  ...
  handleConfirmModalMilestone(): void {
    ...
    if (modalBody && modalBody.formGroup && modalBody.formGroup.valid) {
      ...
      this.milestoneService.saveOrUpdate(milestoneRequest).pipe(takeUntil(this.destroy$)).subscribe({
        next: () => {
  ...
  handleRemoveForm(milestone: MilestoneDTO): void {
    ...
    this.milestoneService.delete(id, this.piaoDTO.id || -1, this.testoSezione).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => {
  ...
  private loadPromemoriaDropdown(): void {
    this.promemoriaService.getAll().pipe(takeUntil(this.destroy$)).subscribe({
      next: (response) => {
