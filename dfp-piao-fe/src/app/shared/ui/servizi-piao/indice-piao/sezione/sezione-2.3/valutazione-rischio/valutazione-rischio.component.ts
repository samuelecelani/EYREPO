...
export class ValutazioneRischioComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  subTitleValutazioneRischioDettaglio: string =
    'SEZIONE_23.VALUTAZIONE_RISCHIO.ATTIVITA_SENSIBILE.SUB_TITLE_DETTAGLIO';
  labelSelectAttivita: string = 'SEZIONE_23.VALUTAZIONE_RISCHIO.ATTIVITA_SENSIBILE.SELECT_LABEL';
  labelSelectAttivitaDettaglio: string =
    'SEZIONE_23.VALUTAZIONE_RISCHIO.ATTIVITA_SENSIBILE.SELECT_LABEL_DETTAGLIO';
  ...
  labelAttivitaSensibileNotFound: string =
    'SEZIONE_23.VALUTAZIONE_RISCHIO.ATTIVITA_SENSIBILE.NOT_FOUND';
  ...
  titleEventiRischiosiDettaglio: string =
    'SEZIONE_23.VALUTAZIONE_RISCHIO.ATTIVITA_SENSIBILE.EVENTI_RISCHIOSI_TITLE_DETTAGLIO';
  ...
  handleAttivitaSensibileChange(event: any, index: number): void {
    ...
    valutazioneGroup?.get('idAttivitaSensibile')?.setValue(event, { emitEvent: false });
  ...
  handleConfirmModalEventiRischiosi(): void {
    if (this.child && this.child.formGroup.valid) {
      ...
      const objRequest = {
        ...data,
        idSezione23: this.piaoDTO.idSezione23,
        idPiao: this.piaoDTO.id,
        testoSezione: this.testoSezione,
        campiModificati: getChangedFields(
          data,
          this.eventoToEdit,
          ['id', 'idAttivitaSensibile', 'key', 'externalId', 'misure'], // campi da escludere dal confronto
          'eventoRischio'
        ),
      };
  ...
  handleRemoveValutazione(index: number): void {
    ...
    if (idAttivitaSensibile) {
      this.eventoRischioService
        .deleteByAttivitaSensibile(idAttivitaSensibile, this.piaoDTO?.id || -1, this.testoSezione)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: () => {
  ...
  handleRemoveForm(data: { element: any; index: number }) {
    if (this.currentValutazioneRischioIndex !== null) {
      ...
      if (eventoRischioso?.id) {
        this.eventoRischioService
          .delete(eventoRischioso.id, this.piaoDTO?.id || -1, this.testoSezione)
          .pipe(takeUntil(this.destroy$))
          .subscribe({
            next: () => {
