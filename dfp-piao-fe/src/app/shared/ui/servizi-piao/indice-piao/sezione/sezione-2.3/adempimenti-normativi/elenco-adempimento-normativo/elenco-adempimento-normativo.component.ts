...
export class ElencoAdempimentoNormativoComponent extends BaseComponent implements OnInit {
  ...
  handleAddElencoAdempimentoNormativo(): void {
    ...
    this.adempimentoNormativoService.save(formValue).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => {
  ...
  handleRemoveForm(adempimento: AdempimentoNormativoDTO) {
    ...
    if (adempimento.id) {
      this.adempimentoNormativoService
        .delete(adempimento.id, this.piaoDTO.id || -1, this.testoSezione)
