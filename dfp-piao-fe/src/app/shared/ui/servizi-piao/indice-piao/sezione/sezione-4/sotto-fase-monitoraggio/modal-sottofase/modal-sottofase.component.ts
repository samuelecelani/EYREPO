...
export class ModalSottofaseComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    // Reinizializza il form quando la modale viene aperta o quando cambia sottoFaseToEdit
    if (changes['open']) {
      if (changes['open'].currentValue === true) {
        // Quando la modale si apre, reinizializza il form
        this.initForm();
      } else if (changes['open'].currentValue === false) {
        // Quando la modale si chiude, resetta il form
        this.resetForm();
      }
    }
    if (changes['sottoFaseToEdit']) {
      // Se la modale è già aperta, reinizializza il form con i nuovi dati
      if (this.open) {
        this.initForm();
      }
    }
  ...
  handleConfirm(): void {
    ...
    const nuovaSottofase: SottofaseMonitoraggioDTO = {
      id: this.sottoFaseToEdit?.id, // Include l'id se presente (per update)
    ...
    // Emetti l'evento con i dati del form
    // Il componente padre gestirà la chiamata API e la chiusura della modale
