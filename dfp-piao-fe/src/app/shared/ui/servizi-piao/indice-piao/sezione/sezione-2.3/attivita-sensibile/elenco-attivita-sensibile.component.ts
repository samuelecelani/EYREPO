...
export class ElencoAttivitaSensibileComponent extends BaseComponent implements OnInit {
  ...
  titleElencoAttivitaDettaglio: string =
    'SEZIONE_23.ATTIVITA_SENSIBILI.TITLE_ELENCO_ATTIVITA_SENSIBILE_DETTAGLIO';
  ...
  handleAddElencoAttivita(): void {
    const formValue = {
      ...
      campiModificati: getChangedFields(
        this.child?.formGroup.value,
        this.attivitaToEdit,
        ['id', 'idSezione23', 'externalId', 'key', 'eventoRischio'], // campi da escludere dal confronto
        'attivitasensibile'
      ),
    };
    this.attivitaSensibileRischioService.save(formValue).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => {
  ...
  handleRemoveForm(element: any) {
    ...
    if (attivita.id) {
      this.attivitaSensibileRischioService
