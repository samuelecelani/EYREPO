...
export class ValorePubblicoAnticorruzioneComponent implements OnInit {
  ...
  /** Flag che indica se è in corso un'operazione di delete */
  isDeletingObiettivo: boolean = false;
  ...
  labelObiettiviPrevenzioneNotFound: string =
    'SEZIONE_23.ANTICORRUZIONE.OBIETTIVI_PREVENZIONE_NOT_FOUND';

  labelObiettivoPrefix: string = 'SEZIONE_23.ANTICORRUZIONE.OBIETTIVO_PREVENZIONE.TITLE';
  labelObiettivoPrefixDettaglio: string =
    'SEZIONE_23.ANTICORRUZIONE.OBIETTIVO_PREVENZIONE.TITLE_DETTAGLIO';
  labelIntroText: string = 'SEZIONE_23.ANTICORRUZIONE.OBIETTIVO_PREVENZIONE.INTRO_TEXT';
  labelIntroTextDettaglio: string =
    'SEZIONE_23.ANTICORRUZIONE.OBIETTIVO_PREVENZIONE.INTRO_TEXT_DETTAGLIO';
  ...
  ngOnInit(): void {
    ...
    this.loadOvpOptions();
    this.savedObiettivi = structuredClone(this.obiettivoPrev.value) as any[]; // Clona il valore iniziale degli obiettivi per confronto futuro
  }
  ...
  private getObiettivoSnapshotJson(obiettivoIndex: number): string | null {
    if (obiettivoIndex >= this.obiettivoPrev.length) return null;
    const fg = this.obiettivoPrev.at(obiettivoIndex) as FormGroup;
  ...
  onDocumentClick(event: MouseEvent): void {
    ...
    // Ignora se ci sono modal aperti o è in corso una delete
    if (this.openModalDelete || this.isDeletingObiettivo) return;
  ...
  /** Salva un singolo obiettivo prevenzione corruzione e trasparenza se dirty e con id */
  private autoSaveSingoloObiettivo(obiettivoIndex: number): void {
    if (obiettivoIndex >= this.obiettivoPrev.length) return;
    ...
    let campiModificati = getChangedFields(
      obj,
      this.savedObiettivi?.[obiettivoIndex],
    ...
    const savedIndicatori = this.savedObiettivi?.[obiettivoIndex]?.indicatori || [];
    ...
    if (
      ...
      campiModificati = campiModificati
        ? campiModificati + `,valorePubblico.indicatori`
        : `valorePubblico.indicatori`;
    }
    ...
    this.obiettivoPrevenzioneCorruzioneTrasparenzaService.save(obiettiviRequest).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: () => {
        fg.markAsPristine();
        this.savedObiettivi = structuredClone(this.obiettivoPrev.value) as any[];
  ...
  handleAddObiettivo(): void {
    ...
    this.obiettivoPrevenzioneCorruzioneTrasparenzaService.save(obiettivoForm.value).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (data: any) => {
  ...
  handleRemoveObiettivo(index: number): void {
    // Reset dello stato di editing per prevenire auto-save durante la delete
    this.isDeletingObiettivo = true;
    this.isEditingObiettivo = false;
    this.editingObiettivoIndex = null;
    this.obiettivoSnapshot = null;
    ...
    if (id) {
      this.obiettivoPrevenzioneCorruzioneTrasparenzaService
        ...
        .pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
          next: () => {
            ...
            this.isDeletingObiettivo = false;
          },
          error: () => {
