...
export class ModalBodyIndicatoreComponent extends ModalBodyComponent implements OnInit {
  ...
  piaoDTO!: PIAODTO;
  Object = Object; // esposto per il template debug
  ...
  removeFormControls(): void {
    // Rimuove dinamicamente tutti i control tranne 'choice' (unico control persistente
    // creato in ngOnInit). Evita di mantenere control orfani con Validators.required
    // che renderebbero il form invalido al cambio di scelta (es. inserisci → recupera).

    this.indicatoreToEdit = {} as IndicatoreDTO; // reset dell'indicatore in modifica per evitare di mostrare dati non coerenti al cambio di scelta

    Object.keys(this.formGroup.controls)
      .filter((controlName) => controlName !== 'choice')
      .forEach((controlName) => {
