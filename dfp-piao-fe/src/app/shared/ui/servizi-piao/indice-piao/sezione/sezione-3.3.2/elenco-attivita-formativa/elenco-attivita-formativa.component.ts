...
export class ElencoAttivitaFormativaComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscription.add(
      this.sezione332Service.onSezione332Updated$.subscribe((sezione332) => {
        if (sezione332) {
  ...
  handleConfirmModal(): void {
    if (this.child?.formGroup) {
      ...
      // Save via service
      this.attivitaFormativeService.save(attivitaFormativaRequest).subscribe({
        next: (saved) => {
          console.log('Attività formativa salvata con successo:', saved);
          ...
          this.sezione332Service.reloadSezione332AndUpdateSession().subscribe();
        },
        error: (err) => {
