...
export class ElencoMonitoraggioMisurePrevenzioneComponent extends BaseComponent implements OnInit {
  ...
  handleAddElencoMonitoraggioMisurePrevenzione(): void {
    const objRequest = {
      ...
      campiModificati: getChangedFields(
        this.child.formGroup.value,
        this.monitoraggioPrevenzioneEdit,
        ['id'], // campi da escludere dal confronto
        `monitoraggioPrevenzione`
      ),
    };

    this.monitoraggioPrevenzioneService.save(objRequest).pipe(takeUntil(this.destroy$)).subscribe({
      next: (data: any) => {
  ...
  handleRemoveForm(element: any) {
    ...
    if (element.modalita.id) {
      this.monitoraggioPrevenzioneService
        .delete(element.modalita.id, this.piaoDTO.id || -1, this.testoSezione)
