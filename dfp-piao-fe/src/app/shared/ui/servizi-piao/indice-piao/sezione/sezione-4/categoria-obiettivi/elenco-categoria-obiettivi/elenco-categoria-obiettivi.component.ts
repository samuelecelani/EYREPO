...
export class ElencoCategoriaObiettiviComponent extends BaseComponent implements OnInit {
  ...
  initDropdownCategoriaObiettivi(codTipologiaFK: string): void {
    this.categoriaObiettiviSerivice.getAllCategoriaObiettiviTip(codTipologiaFK).pipe(takeUntil(this.destroy$)).subscribe({
      next: (data) => {
  ...
  handleSaveCategoriaObiettivo(): void {
    ...
    this.categoriaObiettiviSerivice.save(categoriaRequest).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => {
