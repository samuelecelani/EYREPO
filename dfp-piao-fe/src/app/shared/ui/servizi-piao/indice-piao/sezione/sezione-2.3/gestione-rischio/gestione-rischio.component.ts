...
export class GestioneRischioComponent implements OnInit {
  ...
  subTitleGestioneRischioDettaglio: string =
    'SEZIONE_23.GESTIONE_RISCHIO.EVENTO_RISCHIOSO.SUB_TITLE_DETTAGLIO';
  labelSelectEventoRischioso: string = 'SEZIONE_23.GESTIONE_RISCHIO.EVENTO_RISCHIOSO.SELECT_LABEL';
  labelSelectEventoRischiosoDettaglio: string =
    'SEZIONE_23.GESTIONE_RISCHIO.EVENTO_RISCHIOSO.SELECT_LABEL_DETTAGLIO';
  labelGestioneRischioNotFound: string = 'SEZIONE_23.GESTIONE_RISCHIO.EVENTO_RISCHIOSO.NOT_FOUND';
  ...
  labelMisuraPrevenzioneDettaglio: string =
    'SEZIONE_23.GESTIONE_RISCHIO.EVENTO_RISCHIOSO.MISURE_PREVENZIONE_LABEL_DETTAGLIO';
  ...
  messageNotFoundStakeholder: string =
    'SEZIONE_22.OBBIETTIVI_TRASVERSALI.OBIETTIVO.DETAILS.NOT_FOUND_STAKEHOLDER';
  ...
  ngOnInit(): void {
    ...
    this.savedMisura = structuredClone(this.formGroup.get('gestioneRischio')?.value || []);
  }
  ...
  private autoSaveSingolaMisura(gestioneIndex: number, misuraIndex: number): void {
    ...
    let campiModificati = getChangedFields(
      obj,
      this.savedMisura?.[gestioneIndex]?.[misuraIndex],
      ['id', 'indicatori', 'key', 'externalId', 'idEventoRischio', 'monitoraggioPrevenzione'], // campi da escludere dal confronto
      `misuraEventoRischio`
    );
    ...
    const savedIndicatori = this.savedMisura?.[gestioneIndex]?.[misuraIndex]?.indicatori || [];
    ...
    if (
      ...
      campiModificati = campiModificati
        ? campiModificati + `,misuraEventoRischio.indicatori`
        : `misuraEventoRischio.indicatori`;
    }
    ...
    this.misuraPrevezioneEventoRischioService.save(obiettiviRequest).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: () => {
        fg.markAsPristine();
        this.savedMisura = structuredClone(this.formGroup.get('gestioneRischio')?.value || []);
  ...
  handleAddMisurePrevenzione(index: number): void {
    ...
    this.misuraPrevezioneEventoRischioService.save(misuraPrevenzioneFormGroup.value).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (data: any) => {
  ...
  handleRemoveGestione(index: number): void {
    ...
    if (idEventoRischioso) {
      this.misuraPrevezioneEventoRischioService
        .deleteAllByIdEventoRischioso(idEventoRischioso, this.piaoDTO.id || -1, this.testoSezione)
  ...
  handleRemoveMisuraPrevenzione(indexGestione: number, indexMisura: number): void {
    if (this.getMisuraPrevenzione(indexGestione).at(indexMisura).get('id')?.value) {
      this.misuraPrevezioneEventoRischioService
        .delete(
          this.getMisuraPrevenzione(indexGestione).at(indexMisura).get('id')?.value,
          this.piaoDTO.id || -1,
          this.testoSezione
        )
        .pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
  ...
  getSelectedStakeholders(mPrev: FormGroup): LabelValue[] {
    const selectedIds: number[] = mPrev.get('stakeholders')?.value ?? [];
