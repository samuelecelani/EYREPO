...
import { getCurrentTriennioFull } from '../../../utils/utils';
...
export class CardActionServiziPiaoComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  handleGoToPath(card: CardActionServiziPiao): void {
    ...
    if (card.path === 'modal') {
      ...
      //triennio gestito con triennio anno corrente
      piaoDTO.triennioRiferimento = getCurrentTriennioFull();
      this.piaoService
        .getOrCreatePiao(piaoDTO)
  ...
  handleConfirmModal(): void {
    ...
    piaoDTO.triennioRiferimento = this.child.formGroup.controls['triennio'].value;
  ...
  trackByCardId(_index: number, card: CardActionServiziPiao): string {
    return card.id;
  }
