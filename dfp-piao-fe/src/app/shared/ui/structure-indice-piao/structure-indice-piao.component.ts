...
export class StructureIndicePiaoComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.accountService.getAccount().pipe(takeUntil(this.destroy$)).subscribe((user) => {
      this.isDfp = this.roleRoutingService.isDfpAuthority(user?.typeAuthority);
    });

    if (!this.sezioni) {
      if (this.piaoDTO && this.piaoDTO.id) {
        this.piaoService.getStructureIndicePIAO(this.piaoDTO.id).pipe(takeUntil(this.destroy$)).subscribe({
  ...
  trackByNumeroSezione(_index: number, sezione: StrutturaIndicePiaoDTO): string {
    return sezione.numeroSezione;
  }
}
