...
export class MisureGeneraliComponent implements OnInit {
  ...
  private autoSaveSingolaMisura(misuraIndex: number): void {
    ...
    this.misurePrevenzioneService.save(obiettiviRequest).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: () => {
        fg.markAsPristine();
        this.savedMisure = structuredClone(this.misureGenerali?.value ?? []);
  ...
  handleRemoveMisura(index: number): void {
    ...
    if (misuraId) {
      this.misurePrevenzioneService.delete(misuraId).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
        next: () => {
