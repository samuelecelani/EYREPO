...
export class AdempimentiComponent extends BaseComponent implements OnInit {
  ...
  // Cache delle azioni per adempimento (stessa AdempimentoDTO -> stesso array, stesse callback).
  // Evita la creazione di nuovi riferimenti ad ogni CD cycle che farebbero
  // ri-renderizzare il componente azioni (vertical-ellipsis).
  private actionsCache = new WeakMap<AdempimentoDTO, IVerticalEllipsisActions[]>();

  getActionsFor(adempimento: AdempimentoDTO): IVerticalEllipsisActions[] {
    const cached = this.actionsCache.get(adempimento);
    if (cached) {
      return cached;
    }
    ...
    this.actionsCache.set(adempimento, actions);
    return actions;
  }
  ...
  handleRemoveAdempimento(adempimento: AdempimentoDTO): void {
    ...
    if (adempimentoId) {
      ...
      this.adempimentiService
        .deleteAdempimento(adempimentoId, this.idPiao, this.tipologia, this.testoSezione)
  ...
  handleAddAdempimento(): void {
    ...
    const adempimentoDTO: AdempimentoDTO = {
      ...
      campiModificati: getChangedFields(
        formValue,
        this.adempimentoToEdit,
        ['id', 'idSezione22', 'externalId', 'key', 'tipologia'], // campi da escludere dal confronto
        'adempimento.' + this.tipologia
      ),
    };
    ...
    this.adempimentiService
      .saveOrUpdateAdempimento(adempimentoDTO)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: () => {
