...
export class GraphicIndicePiaoComponent {
  ...
  @Input() statusSezioniGrapich: number[] = [2, 3, 2, 1, 1, 0, 0];
  labelsPie: string[] = [
    ...
    '\nSezioni \ne sottosezioni in richiesta approvazione',
    '\nSezioni \ne sottosezioni pubblicate',
  ];
  colors: string[] = ['#2F475E', '#8700D9', '#008055', '#AF7E00', '#0066CC', '#ff5733', '#003366'];
  ...
  labelsLegend: string[] = [
    ...
    'SCRIVANIA_PA.GRAPHIC_INDICE_PIAO.LABELS.RICHIESTA_APPROVAZIONE',
    'SCRIVANIA_PA.GRAPHIC_INDICE_PIAO.LABELS.PUBBLICATO',
  ];
  ...
  private recompute(): void {
    ...
    const richiestaApprovazione = this.statusSezioniGrapich[5] ?? 0;
    const pubblicato = this.statusSezioniGrapich[6] ?? 0;
    // Se tutte le sezioni sono in richiesta approvazione o pubblicate, le consideriamo come validate
    this.validated =
      this.total > 0 && (richiestaApprovazione === this.total || pubblicato === this.total)
        ? this.total
        : (this.statusSezioniGrapich[this.centerIndex] ?? 0);
  }
