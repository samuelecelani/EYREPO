...
export class AttachmentComponent extends BaseComponent implements OnInit {
  ...
  /**
   * Verifica se l'allegato è un'immagine in base all'estensione del codDocumento
   * (o, in fallback, al campo `type` o alla presenza di base64).
   */
  isImageAttachment(allegatoDto: AllegatoDTO): boolean {
    if (allegatoDto?.base64) {
      return true;
    }
    const imageTypes = ['png', 'jpg', 'jpeg', 'svg', 'gif', 'webp', 'bmp', 'ico', 'tiff', 'tif'];

    // 1) Prova a estrarre l'estensione dal codDocumento (es. "foto.png")
    const codDocumento = allegatoDto?.codDocumento || '';
    const dotIndex = codDocumento.lastIndexOf('.');
    if (dotIndex !== -1 && dotIndex < codDocumento.length - 1) {
      const ext = codDocumento.substring(dotIndex + 1).toLowerCase();
      if (imageTypes.includes(ext)) {
        return true;
      }
    }

    // 2) Fallback sul campo type
    const type = (allegatoDto?.type || '').toLowerCase().replace('.', '');
    return imageTypes.includes(type);
  }

  /**
   * Sceglie la modalità di download corretta in base al tipo di allegato:
   * - immagini -> downloadLogoImage
   * - allegati generici -> download
   */
  handleDownload(allegatoDto: AllegatoDTO): void {
    if (this.isImageAttachment(allegatoDto)) {
      this.downloadLogoImage(allegatoDto);
    } else {
      this.download(allegatoDto?.downloadUrl, allegatoDto?.codDocumentoFE);
    }
  }

  downloadLogoImage(allegatoDto: AllegatoDTO): void {
    if (!allegatoDto.base64) {
      // Fallback: se non è un'immagine base64 usa il download via URL (es. approvazione-pubblicazione)
      if (allegatoDto.downloadUrl) {
        this.download(allegatoDto.downloadUrl, allegatoDto.codDocumentoFE);
      }
      return;
    }
