...
/**
 * Restituisce il triennio corrente in formato "YYYY-YYYY" (es. "2026-2028" nel 2026).
 * Calcolato come (anno corrente) - (anno corrente + 2).
 */
export function getCurrentTriennioFull(): string {
  const year = new Date().getFullYear();
  return `${year}-${year + 2}`;
}
...
export function createFormIndicatoreFromPiaoSession(
  fb: FormBuilder,
  item: IndicatoreDTO,
  pattern: string = INPUT_REGEX
