...
export class DynamicTBTBComponent {
  ...
  @Input() isStakeholder: boolean = false;
  @Input() isIntegrationTeam: boolean = false;
  @Input() isOrganoPolitico: boolean = false;
  ...
  handleRemoveForm(element: any) {
    ...
    if ((!this.isStakeholder && !this.isIntegrationTeam && !this.isOrganoPolitico) || !element?.id) {
      this.arrayForm.removeAt(element.index);
    } else if (element?.id && (this.isStakeholder || this.isIntegrationTeam || this.isOrganoPolitico)) {
      // Emetti l'ID dell'elemento, il parent gestirà la delete via API
