...
export class DropdownComponent
  implements ControlValueAccessor, OnInit, OnChanges, AfterViewChecked, OnDestroy
{
  @Input() dropdown!: LabelValueWithChildren[];
  // Genera un id univoco per ogni istanza per evitare collisioni quando il chiamante
  // non passa esplicitamente [idName]. Senza questo, più dropdown nella stessa pagina
  // condividono lo stesso id e i click sulle <label for="..."> generano scroll improvvisi
  // verso il primo elemento con quell'id presente nel DOM.
  @Input() idName: string = `dropdown-${++DropdownComponent._instanceCounter}`;
  ...
  // Coordinate calcolate per il dropdown in position: fixed (per uscire dallo stacking context dei genitori)
  dropdownTop: number = 0;
  dropdownBottom: number = 0;
  dropdownLeft: number = 0;
  dropdownWidth: number = 0;

  @ViewChild('multiSelectDropdownEl') multiSelectDropdownEl?: ElementRef<HTMLElement>;
  private dropdownPortaledTo: HTMLElement | null = null;
  ...
  // Listener (in fase di capture) per intercettare lo scroll di QUALSIASI contenitore
  // antenato scrollabile, non solo quello della window. Senza capture, gli scroll
  // interni (es. modali o aree con overflow) non vengono rilevati e il dropdown
  // in position: fixed resta "fermo" mentre il trigger si sposta, sovrapponendosi
  // agli altri elementi.
  private readonly onScrollCapture = (event: Event): void => {
    if (!this.isMulti || !this.isDropdownOpen) {
      return;
    }
    // Ignora lo scroll che avviene DENTRO il dropdown stesso (lista di checkbox)
    if (this.dropdownPortaledTo?.contains(event.target as Node)) {
      return;
    }
    this.restoreDropdownToOriginalParent();
    this.isDropdownOpen = false;
  };

  get allHidden(): boolean {
    return !!this.dropdown && this.dropdown.length > 0 && this.dropdown.every((opt) => opt.hidden);
  }
  ...
  onDocumentClick(event: Event): void {
    // Chiude il dropdown multiselect se il click è fuori dal componente e fuori dal dropdown portato in body
    if (
      this.isMulti &&
      this.isDropdownOpen &&
      !this.elementRef.nativeElement.contains(event.target) &&
      !(this.dropdownPortaledTo && this.dropdownPortaledTo.contains(event.target as Node))
    ) {
      this.restoreDropdownToOriginalParent();
      this.isDropdownOpen = false;
    }
  ...
  ngOnInit(): void {
    ...
    // Capture = true così intercetta lo scroll di TUTTI gli antenati scrollabili
    // (non solo della window): chiude il dropdown evitando che resti disallineato.
  ...
  toggleDropdown(): void {
    if (!this.disabled && this.isMulti) {
      ...
      if (!willOpen) {
        // Sta per chiudere: prima ripristina l'elemento al parent originale
        this.restoreDropdownToOriginalParent();
      }
      ...
      if (this.isDropdownOpen) {
        // Calcola posizione e dimensioni del trigger per posizionare il dropdown in position: fixed
        const trigger = this.elementRef.nativeElement.querySelector('.multi-select-trigger');
        if (trigger) {
          const rect = trigger.getBoundingClientRect();
          const dropdownHeight = 300; // max-height del dropdown (18.75rem ≈ 300px)
          ...
          if (this.openDirection === 'down') {
            ...
            this.dropdownTop = 0;
            this.dropdownBottom = viewportHeight - rect.top + 2;
          }
        }
      }
    }
  ...
  onWindowChange(): void {
    // Chiudi il dropdown su scroll/resize per evitare disallineamenti (position: fixed non segue lo scroll del contenuto)
    if (this.isMulti && this.isDropdownOpen) {
      this.restoreDropdownToOriginalParent();
      this.isDropdownOpen = false;
    }
  ...
  ngAfterViewChecked(): void {
    // Sposta la dropdown sotto document.body per evitare di ereditare lo stacking context dei genitori
    if (this.isDropdownOpen && this.multiSelectDropdownEl && !this.dropdownPortaledTo) {
      const el = this.multiSelectDropdownEl.nativeElement;
      this.originalParent = el.parentElement;
      this.originalNextSibling = el.nextSibling;
      this.dropdownPortaledTo = el;
  ...
  /**
   * Riporta la dropdown nel suo parent DOM originale.
   * Va chiamato PRIMA di settare isDropdownOpen = false per evitare
   * che Angular fallisca nel rimuovere l'elemento dall'@if.
   */
  private restoreDropdownToOriginalParent(): void {
    if (this.dropdownPortaledTo && this.originalParent) {
      if (
        this.originalNextSibling &&
        this.originalNextSibling.parentElement === this.originalParent
      ) {
        this.originalParent.insertBefore(this.dropdownPortaledTo, this.originalNextSibling);
      } else {
        this.originalParent.appendChild(this.dropdownPortaledTo);
      }
    }
    this.dropdownPortaledTo = null;
  ...
  trackByOptionValue(_index: number, opt: LabelValueWithChildren): any {
    return opt.value;
  }
}
