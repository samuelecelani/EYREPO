...
export class DatePickerComponent implements OnInit, OnDestroy {
  ...
  @Input() label!: string;
  // Id univoco di default per evitare collisioni quando più date-picker
  // sono presenti nella stessa pagina senza un [id] esplicito.
  @Input() id: string = `piao-date-picker-${++DatePickerComponent._instanceCounter}`;
  ...
  ngOnInit(): void {
    ...
    this.sub = this.control?.valueChanges.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((val) => {
      if (!val) {
        this.displayValue = '';
        return;
      }
