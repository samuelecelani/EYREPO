...
export class ModalAddEntityComponent implements OnChanges {
  ...
  private saveStakeholder(formValue: { field1: string; field2: string }): void {
    ...
    this.stakeholderService.save(stakeholderRequest).subscribe({
      next: (savedEntity) => {
        this.isSaving = false;
  ...
  private savePrioritaPolitica(formValue: { field1: string; field2: string }): void {
    ...
    this.prioritaPoliticaService.save(prioritaPoliticaRequest).subscribe({
      next: (savedEntity) => {
        this.isSaving = false;
  ...
  private saveAreaOrganizzativa(formValue: { field1: string; field2: string }): void {
    ...
    this.areaOrganizzativaService.save(areaOrganizzativaRequest).subscribe({
      next: (savedEntity) => {
        this.isSaving = false;
