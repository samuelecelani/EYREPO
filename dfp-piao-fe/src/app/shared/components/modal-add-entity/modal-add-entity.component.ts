...
export class ModalAddEntityComponent implements OnChanges {
  ...
  private saveStakeholder(formValue: { field1: string; field2: string }): void {
    ...
    const stakeholderRequest = {
      ...
      campiModificati: 'stakeHolders.nomeStakeHolder,stakeHolders.relazionePA',
    };

    this.stakeholderService.save(stakeholderRequest).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (savedEntity) => {
        this.isSaving = false;
  ...
  private savePrioritaPolitica(formValue: { field1: string; field2: string }): void {
    ...
    const prioritaPoliticaRequest = {
      ...prioritaPolitica,
      idSezione1: this.idSezione1!,
      testoSezione: this.testoSezione!,
      campiModificati:
        'prioritaPolitica.nomePrioritaPolitica,prioritaPolitica.descrizionePrioritaPolitica',
    };

    this.prioritaPoliticaService.save(prioritaPoliticaRequest).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (savedEntity) => {
        this.isSaving = false;
  ...
  private saveAreaOrganizzativa(formValue: { field1: string; field2: string }): void {
    ...
    const areaOrganizzativaRequest = {
      ...areaOrganizzativa,
      idSezione1: this.idSezione1!,
      testoSezione: this.testoSezione!,
      campiModificati: 'areeOrganizzative.nomeArea,areeOrganizzative.descrizioneArea',
    };

    this.areaOrganizzativaService.save(areaOrganizzativaRequest).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (savedEntity) => {
        this.isSaving = false;
