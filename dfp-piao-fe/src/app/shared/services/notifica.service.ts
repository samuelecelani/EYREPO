...
export class NotificaService extends BaseComponent {
  ...
  getNotifiche(request: IGetNotificheRequest): Observable<IAllNotificationsResponse | undefined> {
  ...
  private getNotificheSilent(
    request: IGetNotificheRequest
  ): Observable<IAllNotificationsResponse | undefined> {
    return this.getUserContext$().pipe(
      switchMap((user) => {
        request.codiceFiscale = user.cf;
        let userDFP = this.roleRoutingService.isDfpAuthority(user.user.typeAuthority);
  ...
  private async setAllNotifications(): Promise<void> {
    ...
    const obs$: Observable<IAllNotificationsResponse | undefined> = this.getNotifiche(params);

    const res: IAllNotificationsResponse | undefined = await lastValueFrom(obs$);
  ...
  private refreshNotificationsSilent(): void {
    ...
    this.getNotificheSilent(params)
  ...
  private listenForNewNotifications(): void {
    ...
    this.userContextSubscription = this.getUserContext$()
      .pipe(takeUntil(this.destroy$))
      .subscribe((user) => {
        const params: IGetNotificheRequest = {
          idModulo: 'PIAO',
  ...
  private initNotificationsReader(): void {
    ...
    this.notificationsToReadSubject$.pipe(takeUntil(this.destroy$)).subscribe({
  ...
  private async markAsUnread(body: INotificationRead): Promise<void> {
    this.patchUnreadNotifiche(body)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: () => {
