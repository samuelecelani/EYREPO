...
export class DatiPubblicatiService {
  ...
  getByObbligoLegge(idObbligoLegge: number): Observable<DatiPubblicatiDTO[]> {
    return this.http
      .get<
        GenericResponse<DatiPubblicatiDTO[]>
   ...
   */
  delete(id: number, idPiao: number, testoSezione: string): Observable<Sezione23DTO | undefined> {
    const params = new HttpParams()
      .set('idPiao', idPiao.toString())
      .set('testoSezione', testoSezione)
      .set('campiModificati', 'datiPubblicati');
    return this.http
      .delete<GenericResponse<void>>(Path.url(`/dati-pubblicati/${id}`), { params })
      .pipe(switchMap(() => this.sezione23Service.reloadSezione23AndUpdateSession()));
  }
}
