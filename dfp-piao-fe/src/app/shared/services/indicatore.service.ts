...
export class IndicatoreService {
  ...
  deleteIndicatore(
    ...
    if (idPiao) {
      params = params.set('idPiao', idPiao);
    }
    if (campiModificati) {
      params = params.set('campiModificati', campiModificati);
    }
    if (testoSezione) {
      params = params.set('testoSezione', testoSezione);
    }

    return this.http
      .delete<GenericResponse<void>>(Path.url(`/indicatore/${id}`), { params })
      .pipe(map((res) => (res ? res.data : undefined)));
  }
}
