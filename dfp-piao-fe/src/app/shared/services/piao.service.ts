...
export type Tipologia = typeof ONLINE | typeof PDF;
...
export class PIAOService {
  ...
  getOrCreatePiao(piaoDTO: PIAODTO): Observable<PIAODTO | undefined> {
    return this.http
  ...
  redigiPiaoIsAllowed(codPA: string): Observable<boolean> {
    ...
    return this.http
      ...
      .pipe(map((res) => (res ? res.data : false)));
  }
  ...
  findByCodPaFkAndIsCurrent(codPAFK: string, isCurrent: boolean = false): Observable<PIAODTO[]> {
    const params = new HttpParams()
      .set('codPAFK', codPAFK)
      .set('isCurrent', isCurrent);

    return this.http
      .get<GenericResponse<PIAODTO[]>>(Path.url('/piao/findByCodPaFkAndIsCurrent'), {
        params,
      })
