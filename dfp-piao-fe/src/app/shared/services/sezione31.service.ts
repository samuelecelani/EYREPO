...
export class Sezione31Service {
  ...
  ): Observable<GraficoSezione31DTO[]> {
    let params = new HttpParams().set('idEntitaFK', idEntitaFK.toString());
    if (codiceIpa) {
      params = params.set('codiceIpa', codiceIpa);
    }
    if (storageMinerva) {
      params = params.set('storageMinerva', storageMinerva.toString());
    }

    return this.http
      .get<
        GenericResponse<GraficoSezione31DTO[]>
      >(Path.url(`/${CodPathEnum.SEZIONE_3_1}/grafico`), { params })
