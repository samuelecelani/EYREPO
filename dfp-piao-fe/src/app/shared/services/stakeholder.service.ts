...
export class StakeholderService {
   ...
   */
  save(stakeholder: StakeHolderDTO): Observable<StakeHolderDTO | undefined> {
    return this.http
    ...
    if (!this.cache.has(idPiao)) {
      this.cache.set(
        ...
        this.http
          ...
          .pipe(
            map((res) =>
              (res && res.data ? res.data : []).sort((a, b) => (a.id ?? 0) - (b.id ?? 0))
            ),
            shareReplay(1)
          )
      );
  ...
  delete(
    ...
    const params = new HttpParams()
      ...
      .set('campiModificati', 'stakeHolders')
      .set('testoSezione', testoSezione)
      .set('forceDelete', forceDelete.toString());

    return this.http.delete<void>(Path.url(`/stakeholder/${id}`), { params }).pipe(
      tap(() => this.clearCache()),
