...
export class MisuraPrevenzioneEventoRischioService {
  ...
  deleteAllByIdEventoRischioso(idEventoRischioso: number, idPiao: number, testoSezione: string) {
    const params = new HttpParams()
      .set('idPiao', idPiao.toString())
      .set('testoSezione', testoSezione)
      .set('campiModificati', 'misurePrevenzioneEventoRischio');
    return this.http
  ...
  delete(id: number, idPiao: number, testoSezione: string) {
    const params = new HttpParams()
      .set('idPiao', idPiao.toString())
      .set('testoSezione', testoSezione)
      .set('campiModificati', 'misuraEventoRischio');
    return this.http
      .delete(Path.url(`/misura-prevenzione-evento-rischio/${id}`), { params })
      .pipe(switchMap(() => this.sezione23Service.reloadSezione23AndUpdateSession()));
  }
