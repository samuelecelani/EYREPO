...
export class PrioritaPoliticaService {
   ...
   */
  save(prioritaPolitica: PrioritaPoliticaDTO): Observable<PrioritaPoliticaDTO | undefined> {
    return this.http
      .post<
  ...
  /**
   * Elimina una Priorità politica esistente
   * DELETE /priorita-politiche/{id}
   */
  delete(
    id: number,
    idPiao: number,
    ...
    const params = new HttpParams()
      ...
      .set('campiModificati', 'prioritaPolitica')
      .set('forceDelete', forceDelete.toString());
    return this.http.delete<void>(Path.url(`/priorita-politiche/${id}`), { params });
  }
  ...
  private reloadAndUpdateSession(): Observable<PrioritaPoliticaDTO[]> {
    ...
    if (piaoDTO && piaoDTO.idSezione1) {
      return this.getBySezione(piaoDTO.idSezione1).pipe(tap((prioritaPolitiche) => {}));
    } else if (piaoDTO && piaoDTO.id) {
      return this.getByIdPiao(piaoDTO.id).pipe(tap((prioritaPolitiche) => {}));
    }
