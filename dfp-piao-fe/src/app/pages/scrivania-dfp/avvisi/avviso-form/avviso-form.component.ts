...
export class AvvisoFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.amministrazioneService.getTipologieAmministrazioni().pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (tipologie) => {
        this.administrationTypeOptions = tipologie.map((t) => ({ label: t, value: t }));
      },
    });

    this.tipologiaAmministrazioneControl.valueChanges.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => {
    ...
    this.amministrazioneControl.valueChanges.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((value: string) => {
      if (
        this.selectedAmministrazioneLabel !== null &&
    ...
    if (this.avvisoId) {
      ...
      this.alertsService.getByIdData(this.avvisoId).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
        next: (avviso) => {
      ...
      this.alertsService.create({} as AlertsDTO).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
        next: (response) => {
