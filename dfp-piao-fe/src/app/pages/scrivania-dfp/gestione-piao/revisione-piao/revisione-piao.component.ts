...
export class RevisionePiaoComponent extends BaseComponent implements OnInit {
  ...
  onTipologiaChange(_value: unknown): void {
    ...
    this.gestionePiaoService.getAmministrazioneByTipologiaAndDenomAndCode(tipologia).subscribe({
      next: (list) => {
  ...
  search(): void {
    ...
    this.piaoService.getPiaoPubblicatiSearchDFP(codiceIpa, tipologia).subscribe({
