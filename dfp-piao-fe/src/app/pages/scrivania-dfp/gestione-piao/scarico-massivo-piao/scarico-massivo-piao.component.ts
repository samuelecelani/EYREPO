...
import { ONLINE } from '../../../../shared/utils/constants';
...
export class ScaricoMassivoPiaoComponent extends BaseComponent implements OnInit {
  ...
  scaricaPiao(row: ScaricoPiaoRow): void {
    ...
    this.scaricoMassivoService.scaricaDatiScaricoMassivo([row.id], codePa).pipe(takeUntil(this.destroy$)).subscribe({
      next: (data) => {
  ...
  private initDropdownTriennio(): void {
    this.piaoService.getPiaoPubblicati('%', '%', undefined, ONLINE).pipe(takeUntil(this.destroy$)).subscribe({
      next: (response) => {
