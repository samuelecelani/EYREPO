...
export class SollecitiComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.tipologiaControl.valueChanges.subscribe(() => {
  ...
  saveEditorContent(): void {
    console.log('Salvataggio contenuto del sollecito:', this.editorData);
    this.sollecitiService.setValoreFromCodice(TEMPLATE_EMAIL_SOLLECITI, this.editorData).subscribe({
      next: () => {
        console.log('Contenuto del sollecito salvato:', this.editorData);
        this.toastService.success('Contenuto del sollecito salvato con successo.');
      },
      error: (err) => {
        console.log('errore nel save del content editor', err);
      },
    });
  }
  ...
  confermaInvio(): void {
    ...
    const selectedCodePA = this.paginatedRows()
      .filter((r) => r.selected)
      .map((r) => r.codePA);
    console.log('Sollecito inviato - codePA selezionate:', selectedCodePA);
    this.dichiarazioneScadenzaService
      ...
      .subscribe({
        ...
        error: (err) => {
          console.log("errore nell'invio del sollecito", err);
        },
      });
  }
  ...
  search(): void {
    ...
    const codPAFK = this.amministrazioneControl.value
      ? this.amministrazioneSuggestions()
          .map((opt) => opt.value)
          .find((a) => a.denominazioneEnte === this.amministrazioneControl.value)?.codiceIPA
      : undefined;
  ...
  private fetchPage(
    ...
    this.dichiarazioneScadenzaService.searchSollecitiPaged(filters, pageable).subscribe({
      next: (page) => {
