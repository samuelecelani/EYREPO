...
import { INPUT_REGEX, KEY_PA_ATTIVA, KEY_PIAO } from '../../../../../../../shared/utils/constants';
...
import { PARiferimentoDTO } from '../../../../../../../shared/models/classes/pa-riferimento-dto';
...
export class Sezione31Component extends BaseComponent implements OnInit, ISezioneBase {
  ...
  paAttiva!: PARiferimentoDTO;

  ngOnInit(): void {
    this.loadSezione31Data();

    this.paAttiva = this.sessionStorageService.getItem(KEY_PA_ATTIVA);
      ...
      .pipe(
        ...
        switchMap((sezione31) => {
          ...
          if (sezione31?.graficoMinerva) {
            return this.sezione31Service.getGraficoSezione31(
              sezione31.id,
              this.paAttiva?.codePA || '',
              isStatoMinervaAttivo(sezione31?.statoSezione)
            );
          }
    ...
    return this.sezione31Service.getSezione31ByIdPiao(previousPiaoId).pipe(
      ...
      switchMap((sezione31) => {
        if (sezione31) {
          ...
          (sezione31 as any).statoSezione = undefined;
          this.sezione31Data = sezione31;
          success = true;
        }
        if (sezione31?.graficoMinerva) {
  ...
  handleOpenModMinerva() {
    window.open(getValue('publicUrlMinerva') ?? '', '_blank');
  }
