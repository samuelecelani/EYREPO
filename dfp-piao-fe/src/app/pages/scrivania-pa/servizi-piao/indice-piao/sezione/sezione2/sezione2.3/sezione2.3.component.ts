...
export class Sezione23Component extends BaseComponent implements OnInit, OnDestroy, ISezioneBase {
  ...
  @ViewChild('misureGenerali') misureGeneraliComponent!: any;
  ...
  // SubTitleDettaglio Attuazione
  subTitleAttuazioneDettaglio: string = 'SEZIONE_23.ATTUAZIONE_SUB_TITLE_DETTAGLIO';
  // Attuazione label
  ...
  subTitleAnalisiContestoDettaglio: string = 'SEZIONE_23.ANALISI_CONTESTO_SUB_TITLE_DETTAGLIO';
  ...
  subTitlePrevenzioneCorruzioneDettaglio: string =
    'SEZIONE_23.PREVENZIONE_CORRUZIONE_SUB_TITLE_DETTAGLIO';
  ...
  labelIntroMisurePrevenzioneDettaglio: string =
    'SEZIONE_23.MISURE_GENERALI_PREVENZIONE.INTRO_DEFINIZIONE_MISURE_LABEL_DETTAGLIO';
  ...
  subTitleTrasparenzaDettaglio: string = 'SEZIONE_23.TRASPARENZA.SUB_TITLE_TRASPARENZA_DETTAGLIO';
  labelIntroTrasparenza: string = 'SEZIONE_23.TRASPARENZA.DESC_TRASPARENZA_LABEL';
  labelIntroTrasparenzaDettaglio: string =
    'SEZIONE_23.TRASPARENZA.DESC_TRASPARENZA_LABEL_DETTAGLIO';
  ...
  subTitleValorePubblicoAnticorruzioneDettaglio: string =
    'SEZIONE_23.ANTICORRUZIONE.SUB_TITLE_OBIETTIVI_ANTICORRUZIONE_DETTAGLIO';
  ...
  subTitleAttivitaSensibiliDettaglio: string =
    'SEZIONE_23.ATTIVITA_SENSIBILI.SUB_TITLE_ATTIVITA_SENSIBILI_DETTAGLIO';
  ...
  subTitleValutazioneRischioDettaglio: string =
    'SEZIONE_23.VALUTAZIONE_RISCHIO.SUB_TITLE_VALUTAZIONE_RISCHIO_DETTAGLIO';
  ...
  subTitleGestioneRischioDettaglio: string =
    'SEZIONE_23.GESTIONE_RISCHIO.SUB_TITLE_GESTIONE_RISCHIO_DETTAGLIO';
  ...
  titleMonitoraggioDettaglio: string =
    'SEZIONE_23.MONITORAGGIO_MISURE.TITLE_MONITORAGGIO_MISURE_DETTAGLIO';
  subTitleMonitoraggioDettaglio: string =
    'SEZIONE_23.MONITORAGGIO_MISURE.SUB_TITLE_MONITORAGGIO_MISURE_DETTAGLIO';
  ...
  codTipologia: string = SectionEnum.SEZIONE_2_3;
  ...
  ngOnInit(): void {
    const piao = this.sessionStorageService.getItem(KEY_PIAO) as PIAODTO;
    this.isSemplificato = piao?.tipologiaOnline === TipologiaOnline.SEMPLIFICATO;
    ...
    this.subscription.add(
      this.sezione23Service.onSezione23Updated$
  ...
  private loadSezione23Data(): void {
    if (this.piaoDTO?.id) {
      this.sezione23Service
        .getSezione23ByIdPiao(this.piaoDTO.id)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: (data) => {
            if (data) {
            ...
            if (!this.noReloadStato(this.sezione23Data?.statoSezione)) {
              this.saveStoricoStatoSezione(this.sezione23Data || new Sezione23DTO());
            }
          },
          error: () => {
            this.createForm();
  ...
  /**
   * Carica i dati della sezione 2.3 dal PIAO precedente senza modificare il piaoDTO in sessione.
   */
  loadFromPreviousPiao(previousPiaoId: number): Observable<boolean> {
    return this.sezione23Service.getSezione23ByIdPiao(previousPiaoId).pipe(
      map((data) => {
        let success = false;
        if (data) {
      ...
      catchError((err) => {
        console.error('Errore nel caricamento dati sezione 2.3 dal PIAO precedente:', err);
        this.createForm();
        return of(false);
      })
    );
  ...
  validate(testoSezione: string, campiModificati: string): Observable<any> {
    const sezione23Id = this.sezione23Data?.id || this.piaoDTO?.idSezione23 || -1;
    return this.sezione23Service.validation(sezione23Id, testoSezione, campiModificati);
  }
  ...
  private updateRealTimeSavedFormArrays(sezione23: Sezione23DTO): void {
    // Obiettivo prevenzione corruzione e trasparenza
    ...
    this.form.setControl(
      'obblighiLegge',
      this.createObbligoLeggeFormArray(sezione23.obblighiLegge || [])
    );
    ...
    // Se semplificato, rimuovi il vincolo minArrayLength(1) dopo il setControl
    if (this.isSemplificato) {
      const obiettivoArray = this.form.get(
        'obiettivoPrevenzioneCorruzioneTrasparenza'
      ) as FormArray;
      if (obiettivoArray) {
        obiettivoArray.clearValidators();
        obiettivoArray.updateValueAndValidity();
      }
      const attivitaArray = this.form.get('attivitaSensibile') as FormArray;
      if (attivitaArray) {
        attivitaArray.clearValidators();
        attivitaArray.updateValueAndValidity();
      }
    }
    ...
    this.form.setControl(
      ...
      this.createObiettiviFormArray(sezione23.obiettivoPrevenzione || [])
    );

    // Emetti il valore aggiornato del form
    this.formValueChanged.emit(this.form.value);
    this.saveStoricoStatoSezione(sezione23);
  }
  ...
  createForm(): void {
      ...
      allegati: this.fb.array<FormGroup>([
        this.fb.group({
          ...
          createdByNameSurname: [
            sezione23.allegati?.[0]?.createdByNameSurname || null,
            [Validators.maxLength(255)],
          ],
          createdByRole: [
            sezione23.allegati?.[0]?.createdByRole || null,
            [Validators.maxLength(255)],
          ],
        }),
      ]),
    ...
    // Se semplificato, rimuovi il vincolo minArrayLength(1) su obiettivoPrevenzioneCorruzioneTrasparenza e attivitaSensibile
    if (this.isSemplificato) {
      const obiettivoArray = this.form.get(
        'obiettivoPrevenzioneCorruzioneTrasparenza'
      ) as FormArray;
  ...
  private buildAttivitaSensibileForSave(sezione23Data: any): any[] {
    ...
    for (const gestione of gestioneList) {
      ...
      const misure = (gestione.misuraPrevenzione || []).map((m: any) => {
        ...
        const stakeholderIds = m.stakeholders || [];
        const stakeholderTransformed = stakeholderIds.map((selectedId: number) => {
          const existingStakeholder = originalMisura?.stakeholders?.find(
        ...
        return {
          ...
          stakeholders: stakeholderTransformed,
          ...
          eventiRischiosi: this.fb.array<FormGroup>(
            (attivita.eventoRischio || []).map((evento) =>
              this.fb.group({
                ...
                idSezione23: [evento.idSezione23],
            ...
            misuraPrevenzione: this.fb.array<FormGroup>(
              (evento.misure || []).map((misura) =>
                this.fb.group({
                  ...
                  stakeholders: [
                    misura.stakeholders?.map((s) => s.stakeholder?.id).filter((id) => id != null) ||
                      [],
                    [],
                  ],
                  indicatori: (() => {
  ...
  private saveStoricoStatoSezione(sezione23: Sezione23DTO): void {
    const newStatus = this.getSectionStatus();
    if (newStatus !== sezione23.statoSezione) {
      this.storicoStatoSezioneService
        .save({
          idEntitaFK: sezione23.id || -1,
