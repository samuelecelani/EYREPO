...
export class InviaRichiestaApprovazioneComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.richiestaApprovazioneService.getRichiestaApprovazione(this.piaoDTO.id!).subscribe({
      next: (res: any) => {
        if (res) {
          this.createForm();
  ...
  handleInviaRichiestaValidazione() {
    ...
    this.richiestaApprovazioneService.saveRichiestaApprovazione(obj).subscribe({
      next: (res: any) => {
