  ...
  implements OnInit, ISezioneBase
{
  ...
  /**
   * isDettaglio dedicato al solo <piao-invia-richiesta-approvazione>:
   * resta true se isDettaglio del padre lo è già, oppure diventa true quando
   * il PIAO/sezione 5 risulta in stato RICHIESTA_APPROVAZIONE (statoPiao sul DTO
   * di sessione o sui dati di approvazione caricati dal BE).
   * Non influisce sul resto del template (data, url, attachment di pubblicazione).
   */
  get isDettaglioInvia(): boolean {
    const stato = this.piaoDTO?.statoPiao ?? this.approvazioneData?.statoPiao;
    return (
      this.isDettaglio ||
      stato === PiaoStatusEnum.RICHIESTA_APPROVAZIONE ||
      stato === SectionStatusEnum.RICHIESTA_APPROVAZIONE
    );
  }
  ...
  private loadApprovazione() {
    this.approvazioneService.getApprovazione(this.piaoDTO.id!).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        if (res) {
          this.createForm();
