...
import { Observable, Subscription, catchError, map, of, takeUntil } from 'rxjs';
...
export class Sezione21Component extends BaseComponent implements OnInit, OnDestroy, ISezioneBase {
  ...
  descriptionAnalisiContestoDettaglio: string =
    'SCRIVANIA_PA.SERVIZI_PIAO.INDICE_PIAO.SEZIONE.SEZIONE_21.ANALISI_CONTESTO.DESCRIPTION_DETTAGLIO';
  ...
  descriptionAnalisiSwotDettaglio: string =
    'SCRIVANIA_PA.SERVIZI_PIAO.INDICE_PIAO.SEZIONE.SEZIONE_21.ANALISI_SWOT.DESCRIPTION_DETTAGLIO';
  ...
  descriptionObiettiviValorePubblicoDettaglio: string =
    'SCRIVANIA_PA.SERVIZI_PIAO.INDICE_PIAO.SEZIONE.SEZIONE_21.OBIETTIVI_VP.DESCRIPTION_DETTAGLIO';
  ...
  descriptionObiettiviTrasversaliDettaglio: string =
    'SCRIVANIA_PA.SERVIZI_PIAO.INDICE_PIAO.SEZIONE.SEZIONE_21.OBIETTIVI_TRASVERSALI.DESCRIPTION_DETTAGLIO';
  ...
  // Allegati
  codTipologia: string = SectionEnum.SEZIONE_2_1;
  ...
  ngOnInit(): void {
    ...
    this.subscription.add(
      this.sezione21Service.onSezione21Updated$
  ...
  private loadSezione21Data(): void {
    if (this.piaoDTO?.id) {
      this.sezione21Service
        .getSezione21ByIdPiao(this.piaoDTO.id)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: (data) => {
            if (data) {
            ...
            if (!this.noReloadStato(this.sezione21Data?.statoSezione)) {
              this.saveStoricoStatoSezione(this.sezione21Data || new Sezione21DTO());
            }
          },
          error: () => {
            this.createForm();
  ...
  /**
   * Carica i dati della sezione 2.1 dal PIAO precedente senza modificare il piaoDTO in sessione.
   */
  loadFromPreviousPiao(previousPiaoId: number): Observable<boolean> {
    return this.sezione21Service.getSezione21ByIdPiao(previousPiaoId).pipe(
      map((data) => {
        let success = false;
        if (data) {
      ...
      catchError((err) => {
        console.error('Errore nel caricamento dati sezione 2.1 dal PIAO precedente:', err);
        this.createForm();
        return of(false);
      })
    );
      ...
      allegati: this.fb.array<FormGroup>([
        this.fb.group({
          ...
          createdByNameSurname: [
            sezione21.allegati?.[0]?.createdByNameSurname || null,
            [Validators.maxLength(255)],
          ],
          createdByRole: [
            sezione21.allegati?.[0]?.createdByRole || null,
            [Validators.maxLength(255)],
          ],
        }),
      ]),
  ...
  reloadForm(sezione21: Sezione21DTO): void {
    ...
    this.formValueChanged.emit(this.form.value);
    this.saveStoricoStatoSezione(sezione21);
  }
  ...
  validate(testoSezione: string, campiModificati: string): Observable<any> {
    ...
    return this.sezione21Service.validation(sezione21Id, testoSezione, campiModificati);
  }
  ...
  private saveStoricoStatoSezione(sezione21: Sezione21DTO): void {
    const newStatus = this.getSectionStatus();
    if (newStatus !== sezione21.statoSezione) {
      this.storicoStatoSezioneService
        .save({
          idEntitaFK: sezione21.id || -1,
