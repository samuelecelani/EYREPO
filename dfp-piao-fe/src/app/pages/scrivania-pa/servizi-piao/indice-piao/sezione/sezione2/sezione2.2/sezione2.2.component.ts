...
import { Observable, Subscription, catchError, forkJoin, map, of, takeUntil } from 'rxjs';
...
export class Sezione22Component extends BaseComponent implements OnInit, OnDestroy, ISezioneBase {
  ...
  descriptionCicloPerformanceDettaglio: string =
    'SEZIONE_22.DESCRIPTION_CICLO_PERFORMANCE_DETTAGLIO';
  ...
  descriptionObiettiviPerformancePADettaglio: string =
    'SEZIONE_22.DESCRIPTION_OBIETTIVI_PERFORMANCE_PA_DETTAGLIO';
  ...
  subTitleAdempimentiDettaglio: string = 'SEZIONE_22.ADEMPIMENTI.SUB_TITLE_ADEMPIMENTI_DETTAGLIO';
  ...
  subTitleObbiettiviTrasversaliDettaglio: string =
    'SEZIONE_22.OBBIETTIVI_TRASVERSALI.SUB_TITLE_OBIETTIVI_TRASVERSALI_DETTAGLIO';
  ...
  descriptionCollegamentoPerformanceDettaglio: string =
    'SEZIONE_22.PERFORMANCE_ORGANIZZATIVA_DESCRIPTION_DETTAGLIO';
  ...
  descriptionPerformanceIndividualeDettaglio: string =
    'SEZIONE_22.DESCRIPTION_PERFORMANCE_INDIVIDUALE_DETTAGLIO';
  ...
  codTipologia: string = SectionEnum.SEZIONE_2_2;
  ...
  ngOnInit(): void {
    ...
    this.subscription.add(
      this.sezione22Service.onSezione22Updated$
  ...
  private loadSezione22Data(): void {
    if (this.piaoDTO?.id) {
      ...
      forkJoin([sezione22$, ovp$])
        .pipe(takeUntil(this.destroy$))
        .subscribe(([sezione22Data, ovpList]) => {
          ...
          this.createForm();
          if (!this.noReloadStato(this.sezione22Data?.statoSezione)) {
            this.saveStoricoStatoSezione(this.sezione22Data || new Sezione22DTO());
          }
        });
    } else {
      this.createForm();
  ...
  /**
   * Carica i dati della sezione 2.2 dal PIAO precedente senza modificare il piaoDTO in sessione.
   */
  loadFromPreviousPiao(previousPiaoId: number): Observable<boolean> {
    const sezione22$ = this.sezione22Service
      .getSezione22ByIdPiao(previousPiaoId)
      .pipe(catchError(() => of(null)));
    const ovp$ = this.ovpService.getAllOvpByIdPiao(previousPiaoId).pipe(
      catchError((err) => {
        console.error('Errore nel caricamento degli OVP dal PIAO precedente:', err);
        return of([] as OVPDTO[]);
      })
    );

    return forkJoin([sezione22$, ovp$]).pipe(
      map(([sezione22Data, ovpList]) => {
        let success = false;
        if (sezione22Data) {
          // Non considerare lo statoSezione del PIAO precedente
          (sezione22Data as any).statoSezione = undefined;
          this.sezione22Data = sezione22Data;
          success = true;
        }
        this.ovpList = ovpList ? ovpList : [];
      ...
      allegati: this.fb.array<FormGroup>([
        this.fb.group({
          ...
          createdByNameSurname: [
            sezione22.allegati?.[0]?.createdByNameSurname || null,
            [Validators.maxLength(255)],
          ],
          createdByRole: [
            sezione22.allegati?.[0]?.createdByRole || null,
            [Validators.maxLength(255)],
          ],
        }),
      ]),
      codTipologia: this.fb.control<string>(SectionEnum.SEZIONE_2_2),
  ...
  private reloadObbiettiviInPlace(obiettivi: any[]): void {
    // Raggruppa per tipologia (stesso pattern di createObbiettiviPerformanceMap)
    const grouped = new Map<TipologiaObbiettivo, any[]>();
    Object.values(TipologiaObbiettivo).forEach((t) => grouped.set(t as TipologiaObbiettivo, []));
    obiettivi.forEach((o) => {
      const tip = TipologiaObbiettivo[o.tipologia as keyof typeof TipologiaObbiettivo];
      if (tip !== undefined && grouped.has(tip)) grouped.get(tip)!.push(o);
    });

    grouped.forEach((items, tipologia) => {
      const existing = this.obbiettiviPerformanceMap.get(tipologia);
      const fresh = createFormArrayObiettivoPerformanceFromPiaoSession(
        this.fb,
        items,
        INPUT_REGEX,
        this.piaoDTO.id || 0
      ...
      if (!existing) {
        // Prima volta: registra e basta
        if (fresh) {
          this.obbiettiviPerformanceMap.set(tipologia, fresh);
          this.form.setControl(`obbiettivi_${tipologia}`, fresh);
        }
        return;
      }
      // In-place: svuota e ripopola SENZA cambiare reference
      existing.clear({ emitEvent: false });
      fresh?.controls.forEach((c) => existing.push(c, { emitEvent: false }));
  ...
  reloadForm(sezione22?: Sezione22DTO): void {
    ...
    this.reloadObbiettiviInPlace(data.obbiettiviPerformance || []);
    this.form.updateValueAndValidity();
    this.isFormReady = true;
    this.formValueChanged.emit(this.form.value);
    this.saveStoricoStatoSezione(this.sezione22Data || new Sezione22DTO());
  ...
  validate(testoSezione: string, campiModificati: string): Observable<any> {
    ...
    return this.sezione22Service.validation(sezione22Id, testoSezione, campiModificati);
  }
  ...
  private _stakeholderOptions: LabelValue[] = [];
  private _stakeholderOptionsSource: any[] | undefined;

  get stakeholderOptions(): LabelValue[] {
    const source = this.piaoDTO?.stakeHolders;
    if (!source) {
      return [];
    }
    if (source !== this._stakeholderOptionsSource) {
      this._stakeholderOptionsSource = source;
      this._stakeholderOptions = source.map((stakeholder: any) => ({
        label: stakeholder.nomeStakeHolder || '',
        value: stakeholder.id || 0,
      }));
    }
    return this._stakeholderOptions;
  }
  ...
  private saveStoricoStatoSezione(sezione22: Sezione22DTO): void {
    const newStatus = this.getSectionStatus();
    if (newStatus !== sezione22.statoSezione) {
      this.storicoStatoSezioneService
        .save({
          idEntitaFK: sezione22.id || -1,
