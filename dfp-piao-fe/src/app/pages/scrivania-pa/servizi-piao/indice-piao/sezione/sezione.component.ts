...
export class SezioneComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  private updateIsDettaglio(): void {
    console.log('Updating isDettaglio with activeSectionId:', this.activeSectionId);
    this.isDettaglio =
  ...
  private checkFormChanged(): boolean {
    ...
    console.log('Il form è cambiato?', hasChanged);
  ...
  handleConfermaPubblicazione() {
    if (this.child?.formGroup && this.approvazionePubblicazioneComponent) {
      console.log('Dati da pubblicare:', this.child.formGroup.value);
      const approvazioneData = {
        ...this.approvazionePubblicazioneComponent.prepareDataForSave(),
        ...this.child.formGroup.value,
      };
      console.log('Dati di approvazione:', approvazioneData);
      this.approvazioneService.saveApprovazione(approvazioneData).subscribe({
        next: (value: any) => {
          this.loadStep();
  ...
  private getIdSezioneStorico(): number {
    switch (this.activeSectionId) {
      ...
      case '4':
        console.log('ID sezione 4:', this.piaoDTO.idSezione4);
  ...
  handleOpenModalStoricoSezione() {
    console.log('Sono dentro handleOpenModalStoricoSezione');
  ...
  handleRecuperaInfoPiao(): void {
    ...
    this.piaoService.getPiaoPrecedente(codPAFK).subscribe({
      next: (piaoPrecedente: PIAODTO | null) => {
        ...
        activeSezione.loadFromPreviousPiao(piaoPrecedente.id).subscribe({
          next: (success) => {
            console.log('Recupero info PIAO precedente, successo:', success);
            if (success) {
