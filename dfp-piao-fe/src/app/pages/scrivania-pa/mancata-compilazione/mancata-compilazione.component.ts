...
export class MancataCompilazioneComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  sezione: SectionEnum = SectionEnum.SEZIONE_MANCATA_COMPILAZIONE;
  ...
  isDettaglioStorico: boolean = false;
  denominazioneAmministrazione: string | null = null;
  denominazioneControl = new FormControl<string | null>({ value: null, disabled: true });

  ngOnInit(): void {
    // Modalità "Dettaglio storico" (DFP) attivata da route data
    this.isDettaglioStorico = !!this.activatedRoute.snapshot.data?.['isDettaglioStorico'];

    // Snapshot di history.state e immediata pulizia: evita che i flag
    // vengano riapplicati dopo reload (Edge/BFCache preserva history.state).
    const navState: Record<string, unknown> = { ...(history.state ?? {}) };
    const stateAmministrazione = navState['amministrazione'] as string | undefined;
    const stateIdPiao = navState['idPiao'] as string | number | undefined;
    const stateCodePA = navState['codePA'] as string | undefined;
    const {
      amministrazione: _a,
      idPiao: _i,
      codePA: _c,
    ...
    if (this.isDettaglioStorico) {
      this.title = 'MANCATA_COMPILAZIONE.DETTAGLIO_TITLE';
      this.subTitle = 'MANCATA_COMPILAZIONE.DETTAGLIO_SUB_TITLE';
      this.isDettaglio = true;
      this.denominazioneAmministrazione = stateAmministrazione ?? null;
      this.denominazioneControl.setValue(this.denominazioneAmministrazione);
    }
    ...
    this.getPaRiferimento$()
      .pipe(
        switchMap((pa) => {
          const dichiarazione$ = stateIdPiao
            ? this.dichiarazioneService.getDichiarazioneScadenzaByPiao(Number(stateIdPiao))
            : this.dichiarazioneService.getExistingDichiarazioneScadenza(
                stateCodePA ? stateCodePA : pa.codePA
              );
          return forkJoin({
      ...
      .subscribe({
        next: ({ motivazioni, dichiarazione }) => {
          ...
          if (dichiarazione && dichiarazione.id !== null && dichiarazione.id !== undefined) {
            ...
            this.idPiao = dichiarazione?.idPiao ?? 0;
            this.isDettaglio = !this.hasFunzionalita('SCR_COMPILAZIONE_MANCATA_INPUT_FORM');
            this.form.patchValue({
              idPiao: this.idPiao,
            });
          }
  ...
  handleValidationSection(): void {
    if (this.form.valid) {
      ...
      this.dichiarazioneService
        .save(dto)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: (response: DichiarazioneScadenzaDTO | undefined) => {
