...
export class HelpDeskComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  setupAttachmentForTicket() {
    ...
    this.attachmentComponent.getAllAttachment = () => {
      ...
      this.ticketService.getAllegatiByTicket(idTicket).subscribe({
        next: (allegati) => {
    ...
    this.attachmentComponent.handleConfirmModal = () => {
      ...
      console.log('Dati allegato da salvare:', modalForm, modalForm.value);
      ...
      this.ticketService.saveAllegato(allegato, modalForm.controls['file'].value).subscribe({
        next: (data) => {
          if (data.idTicketFk) {
    ...
    this.attachmentComponent.remove = (idDoc: number) => {
      this.ticketService.deleteAllegato(idDoc).subscribe({
        next: () => {
  ...
  getCategorieByIdModulo() {
    this.ticketService.getCategorieByIdModulo('PIAO').subscribe((categorie) => {
