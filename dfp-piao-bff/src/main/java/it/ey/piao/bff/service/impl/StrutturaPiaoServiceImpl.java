...
public class StrutturaPiaoServiceImpl implements IStrutturaPiaoService {
    ...
    @Override
    public Mono<GenericResponseDTO<Void>> accettaValidazioneSezioniSelezionate(Long idPiao, Map<String,Long> idSezione) {
        HttpHeaders headers = new HttpHeaders();
        headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
        headers.setContentType(MediaType.APPLICATION_JSON);

        String uri = UriComponentsBuilder
            .fromPath("/struttura/validazione/accetta-selezionate")
            .queryParam("idPiao", idPiao)
            .build()
            .toUriString();

        return webClientService
            .patch(uri, webServiceType, idSezione, headers, Void.class)
            .thenReturn(new GenericResponseDTO<Void>() {{
                setData(null);
                setStatus(Status.builder().isSuccess(true).build());
            }});
    }
}
