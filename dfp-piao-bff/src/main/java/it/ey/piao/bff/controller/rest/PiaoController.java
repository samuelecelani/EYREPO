...
//TODO: Esempio di come rendere accessibile l'api su base di un ruolo globale oppure ruolo per singole sezioni
   //@PreAuthorize("hasRole('VALIDATORE') or hasRole('VALIDATORE_' + SEZIONE1)")
    ...
    @GetMapping("/findByCodPaFkAndIsCurrent")
    public Mono<ResponseEntity<GenericResponseDTO<List<PiaoDTO>>>> findByCodPaFkAndIsCurrent(
            @RequestParam String codPAFK,
            @RequestParam(required = false, defaultValue = "false") boolean isCurrent) {
        return piaoService.findByCodPaFkAndIsCurrent(codPAFK, isCurrent).map(ResponseEntity::ok);
    }
    ...
    public Mono<ResponseEntity<GenericResponseDTO<List<PiaoDTO>>>> consultazionePiao(
                                                                                ...
                                                                                @RequestParam(required = false) Tipologia tipologia) {
        return piaoService.consultazionePiao(codPAFK, denominazione, versione, tipologia).map(ResponseEntity::ok);
    }
