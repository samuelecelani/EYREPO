...
public class AuthenticationService {
    ...
    public Mono<String> logout(String sessionId, String redirectUri) {
        ...
        String idToken = loginSession.getToken().getIdToken();
        UriComponentsBuilder logoutUriBuilder = UriComponentsBuilder
            .fromUriString(props.getOauth2Url() + "/protocol/openid-connect/logout")
            .queryParam("client_id", props.getClientId())
            .queryParam("post_logout_redirect_uri", redirectUri);

        // Aggiungi id_token_hint solo se presente
        if (idToken != null && !idToken.isBlank()) {
            logoutUriBuilder.queryParam("id_token_hint", idToken);
        }

        String logoutUri = logoutUriBuilder.build().toUriString();
        log.debug("Logout URI: {}", logoutUri);

        // Revoca il refresh token su Keycloak (best effort - non blocca il logout se fallisce)
        String refreshToken = loginSession.getToken().getRefreshToken();
        if (refreshToken != null && !refreshToken.isBlank()) {
            return oAuth2Service.revokeToken(refreshToken)
                .doOnError(e -> log.warn("Errore durante la revoca del token: {}", e.getMessage()))
                .onErrorResume(e -> Mono.empty()) // Ignora errori di revoca
                .thenReturn(logoutUri);
        }
