...
public class StatoSezioneServiceImpl implements IStatoSezioneService {
    ...
    private static final Logger log = LoggerFactory.getLogger(StatoSezioneServiceImpl.class);

    public StatoSezioneServiceImpl(WebClientService webClientService) {
    ...
    public Mono<GenericResponseDTO<List<StatoSezioneDTO>>> getStatoSezione() {
        ...
        return webClientService.get("/stato/sezione", webServiceType,headers,new ParameterizedTypeReference<List<StatoSezioneDTO>>() {})
            .doOnNext(response -> log.info(" Numero stati ricevuti: {}", response.size()))
            .map(statoList -> {
                GenericResponseDTO<List<StatoSezioneDTO>> finalResponse = new GenericResponseDTO<>();
            ...
            .doOnError(e -> {
                ...
                GenericResponseDTO<StatoSezioneDTO> finalResponse = new GenericResponseDTO<>();
                finalResponse.setStatus(new Status());
