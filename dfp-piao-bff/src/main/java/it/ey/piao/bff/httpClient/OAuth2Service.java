...
public class OAuth2Service {
    ...
    /**
     * Revoca un token (refresh_token) su Keycloak.
     * Questo invalida la sessione lato Keycloak.
     */
    public Mono<Void> revokeToken(String refreshToken) {
        return webClient.post()
            .uri(props.getOauth2Url() + "/protocol/openid-connect/revoke")
            .contentType(MediaType.APPLICATION_FORM_URLENCODED)
            ...
            .onStatus(status -> status.isError(), clientResponse ->
                clientResponse.bodyToMono(String.class)
                    .doOnNext(body -> log.error("Errore Keycloak revoke: {}", body))
                    .then(Mono.error(new RuntimeException("Errore revoke token endpoint")))
            )
            .bodyToMono(Void.class)
            .timeout(Duration.ofSeconds(10))
            .doOnSubscribe(sub -> log.info("Chiamata revoke token iniziata"))
            .doOnSuccess(v -> log.info("Token revocato con successo"))
            .doOnError(err -> log.error("Errore WebClient revoke: {}", err.getMessage(), err));
    }
