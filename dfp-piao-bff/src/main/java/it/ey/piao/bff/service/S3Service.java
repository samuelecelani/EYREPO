...
public class S3Service {
    ...
    @Value("${aws.s3.bucket-definitive}")
    private String S3bucketDefinitive;
    ...
    /**
     * Bucket di scrittura:
     *  - antivirus.enabled = true  -> staging (il file verrà spostato nel definitivo dopo la scansione)
     *  - antivirus.enabled = false -> definitivo (scrittura diretta)
     */
    private String writeBucket() {
        return antivirusEnabled ? S3bucketName : S3bucketDefinitive;
    }

    /**
     * Bucket di lettura: SEMPRE il definitivo.
     */
    private String readBucket() {
        return S3bucketDefinitive;
    }
     ...
     * Legge SEMPRE dal bucket definitivo (la lettura non dipende dal flag antivirus).
     */
    public Mono<String> generatePresignedUrl(String fileKey) {
        return generatePresignedUrlFromBucket(fileKey, readBucket());
    }

    /**
     * Genera Presigned URL reattiva dal bucket di staging (sempre).
     */
    public Mono<String> generatePresignedUrlFromStaging(String fileKey) {
        return generatePresignedUrlFromBucket(fileKey, S3bucketName);
    }

    /**
     * Genera Presigned URL reattiva dal bucket definitivo.
     */
    public Mono<String> generatePresignedUrlFromDefinitive(String fileKey) {
        return generatePresignedUrlFromBucket(fileKey, S3bucketDefinitive);
    }

    private Mono<String> generatePresignedUrlFromBucket(String fileKey, String bucketName) {
    ...
    /**
     * Verifica reattiva dell'esistenza di un oggetto su S3 (HEAD request, no body).
     * Ritorna {@code true} se l'oggetto esiste, {@code false} se NoSuchKey o key vuota.
     * Eventuali altri errori vengono propagati.
     */
    public Mono<Boolean> doesObjectExist(String fileKey) {
        if (fileKey == null || fileKey.isBlank()) {
