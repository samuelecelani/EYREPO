...
public class PdfGenerationServiceImpl implements IPdfGenerationService {
    ...
    private Mono<Map<String, Object>> loadPiao(Long idPiao, String codicePa) {
        return piaoService.findPiaoByCodPAFK(codicePa)
            .flatMap(response -> {
                Map<String, Object> dataMap = new LinkedHashMap<>();
                dataMap.put("idPiao", idPiao);
                if (response.getData() != null) {
                    response.getData().stream()
                        .filter(p -> p.getId() != null && p.getId().equals(idPiao))
                        .findFirst()
                        .ifPresent(piao -> dataMap.put("piao", piao));
                }
                if (!dataMap.containsKey("piao")) {
                    log.warn("PIAO con id={} non trovato per codicePa={}", idPiao, codicePa);
                }
                return Mono.just(dataMap);
            })
            .onErrorResume(e -> {
                log.warn("Errore recupero PIAO per codicePa={}: {}", codicePa, e.getMessage());
                Map<String, Object> dataMap = new LinkedHashMap<>();
                dataMap.put("idPiao", idPiao);
                return Mono.just(dataMap);
            });
