...
public class Sezione31Controller
{
    ...
    @GetMapping("/grafico")
    public Mono<ResponseEntity<GenericResponseDTO<List<GraficoSezione31DTO>>>> getGraficoSezione31Mock(@RequestParam Long idEntitaFK,
                                                                                                       @RequestParam String codiceIpa,
                                                                                                       @RequestParam(required = false) Boolean storageMinerva) {
        return sezione31Service.getGraficoSezione31Mock(idEntitaFK, codiceIpa, storageMinerva).map(ResponseEntity::ok);
    }
}
