...
public class AttoreController {
    ...
    public Mono<ResponseEntity<GenericResponseDTO<AttoreDTO>>> findByExternalIdAndTipoSezione(
        ...
        return attoreService.findByExternalIdAndTipoSezione(externalId, tipoSezione, externalIdFK)
            .map(ResponseEntity::ok);
    }

    @GetMapping("/piao/{idPiao}/sezione/{tipoSezione}")
    public Mono<ResponseEntity<GenericResponseDTO<List<AttoreDTO>>>> findByIdPiaoAndTipoSezione(
            @PathVariable Long idPiao,
            @PathVariable Sezione tipoSezione) {
        return attoreService.findByIdPiaoAndTipoSezione(idPiao, tipoSezione)
            .map(ResponseEntity::ok);
    }
}
