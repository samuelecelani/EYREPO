...
public class Sezione1ServiceImpl implements ISezione1Service {
    ...
    public Mono<GenericResponseDTO<Sezione1DTO>> findByPiao(Long idPiao,String codiceFiscale) {
        ...
        return webClientService.get("/sezione1/"+ idPiao ,webServiceType,headers,Sezione1DTO.class)
            ...
            .flatMap(sezione1 -> {
                ...
                Mono<AnagraficaDTO> dbMono = getAnagraficaFromDb(idPiao);

                boolean cfPresente = codiceFiscale != null && !codiceFiscale.isBlank();

                // Caso 1: nessun CF -> solo chiamata DB
                if (!cfPresente) {
                    log.info("CodiceFiscale non fornito: recupero AnagraficaDTO solo da DB per idPiao={}", idPiao);
                    return dbMono
                        .map(dbAnagrafica -> {
                        ...
                        .onErrorResume(e -> {
                            log.error("Errore recupero anagrafica DB per idPiao={}: {}", idPiao, e.getMessage(), e);
                ...
                return Mono.zip(
                    ...
                    .map(tuple -> {
                        ...
                        if (dbAnagrafica.getIdPiao() != null) merged.setIdPiao(dbAnagrafica.getIdPiao());
                        ...
                        log.info("AnagraficaDTO arricchita con campi DB per idPiao={}", idPiao);
    ...
    /**
     * Recupera l'AnagraficaDTO dal DB (BE) tramite idPiao.
     * Usata per popolare i campi non forniti da IPA: nomeRPCT, cognomeRCTP,
     * ruoloRPCT, dataNominaRPCT, indirizzoURP, telefono.
     */
    private Mono<AnagraficaDTO> getAnagraficaFromDb(Long idPiao) {
        log.info("Recupero anagrafica da DB per idPiao={}", idPiao);
        HttpHeaders headers = new HttpHeaders();
        headers.set("Accept", "application/json");
        return webClientService.get(
                "/anagrafica/by-piao/" + idPiao,
                webServiceType,
                headers,
                AnagraficaDTO.class)
            .doOnNext(r -> log.info("Anagrafica DB ricevuta per idPiao={}: {}", idPiao, r))
            .onErrorResume(e -> {
                log.error("Errore recupero anagrafica DB per idPiao={}: {}", idPiao, e.getMessage(), e);
                return Mono.empty();
            });
