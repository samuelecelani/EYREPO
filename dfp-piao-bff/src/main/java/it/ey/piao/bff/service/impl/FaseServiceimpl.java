...
public class FaseServiceimpl implements IFaseService {
    ...
    @Override
    public Mono<GenericResponseDTO<Void>> deleteFase(Long id, String campiModificati, Long idPiao, String testoSezione) {
        log.info("Richiesta di cancellazione fase con id: {}", id);

        HttpHeaders headers = new HttpHeaders();
        headers.set("Accept", "application/json");

        StringBuilder url = new StringBuilder("/fase/" + id + "?");
