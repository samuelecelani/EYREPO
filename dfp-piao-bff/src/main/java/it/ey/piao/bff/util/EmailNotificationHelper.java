...
public class EmailNotificationHelper {
    ...
    public Mono<Void> sendEmailApprovazione(String toEmail, Long idPiao) {
        return sendEmailApprovazione(toEmail, idPiao, null);
    }

    /**
     * Variante con CC: invia l'email di approvazione PIAO con un destinatario in copia conoscenza.
     *
     * @param toEmail indirizzo email del destinatario
     * @param idPiao  id del PIAO da cui recuperare le informazioni
     * @param ccEmail indirizzo email da mettere in CC (nullable/blank → ignorato)
     */
    public Mono<Void> sendEmailApprovazione(String toEmail, Long idPiao, String ccEmail) {
        log.info("Preparazione email approvazione PIAO a {} (cc={}) per idPiao={}", toEmail, ccEmail, idPiao);
        ...
        return Mono.zip(piaoMono, mailObjectMono, mailBodyMono, urlBaseMono, fromAddressMono, fromNameMono)
            .flatMap(tuple -> {
                ...
                EmailTaskMessageDTO emailTask = EmailTaskMessageDTO.builder()
                    ...
                    .toAddresses(List.of(toAddress))
                    .ccAddresses(
                        (ccEmail != null && !ccEmail.isBlank() && !ccEmail.equalsIgnoreCase(toEmail))
                            ? List.of(MailAddressDTO.builder().email(ccEmail).name(ccEmail).build())
                            : null
                    )
                    .mailObject(mailObject)
                    .mailBody(mailBody)
