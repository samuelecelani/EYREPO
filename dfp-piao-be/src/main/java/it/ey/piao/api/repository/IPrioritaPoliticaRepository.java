...
public interface IPrioritaPoliticaRepository extends BaseRepository<PrioritaPolitica, Long> {
    ...
    /**
     * Verifica se esistono OVP collegati a una priorità politica.
     */
    @Query("SELECT COUNT(opp) > 0 FROM OVPPrioritaPolitica opp WHERE opp.prioritaPolitica.id = :idPriorita")
    boolean existsOvpDependencyByPrioritaPoliticaId(@Param("idPriorita") Long idPriorita);


    @Modifying
    @Query("""
    UPDATE PrioritaPolitica pp
    SET pp.active = false,
        pp.deactivationTime = :deactivationTime
    WHERE pp.id = :id
    """)
