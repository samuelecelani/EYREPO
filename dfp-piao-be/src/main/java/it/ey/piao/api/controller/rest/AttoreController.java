...
public class AttoreController {
    ...
    public ResponseEntity<AttoreDTO> findByExternalIdAndTipoSezione(
            ...
            @RequestParam(required = false) Long externalIdFK) {
        AttoreDTO result = attoreService.findByExternalIdAndTipoSezione(externalId, tipoSezione, externalIdFK);
        return ResponseEntity.ok(result);
    }

    @GetMapping("/piao/{idPiao}/sezione/{tipoSezione}")
    public ResponseEntity<List<AttoreDTO>> findByIdPiaoAndTipoSezione(
            @PathVariable Long idPiao,
            @PathVariable Sezione tipoSezione) {
        return ResponseEntity.ok(attoreService.findByIdPiaoAndTipoSezione(idPiao, tipoSezione));
    }
}
