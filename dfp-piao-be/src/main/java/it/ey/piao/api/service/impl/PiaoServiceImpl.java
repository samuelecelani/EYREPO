...
public class PiaoServiceImpl implements IPiaoService {
    ...
    public PiaoDTO getOrCreatePiao(PiaoDTO piao, String triennioRiferimento) {
        try {
            ...
            if (triennioRiferimento != null && !triennioRiferimento.isBlank()) {
                piao.setTriennioRiferimento(triennioRiferimento);
                log.info("Triennio di riferimento impostato da request param: {}", triennioRiferimento);
            }
            ...
            if (nextYear) {
                ...
                if (piaoNextYear == null) {
                    ...
                    return createNewVersion(piao, piaoNextYear, true, triennioRiferimento);
                }
            }
            ...
            return createNewVersion(piao, existing, false, triennioRiferimento);
     ...
     *
     * @param piaoDTO              il DTO con i dati di input (codPAFK, tipologia, ecc.)
     ...
     * @param triennioRiferimento  il triennio di riferimento passato dall'utente (es. "2027-2029")
     * @return il nuovo PiaoDTO creato con versione incrementata
     */
    private PiaoDTO createNewVersion(PiaoDTO piaoDTO, Piao published, boolean nextYear, String triennioRiferimento) {
        PiaoDTO response = null;
        try {
             ...
             *  Attualmente commentato in vista della nuova versione questi dati verranno scelti dall'utente e non ripresi dalla vecchia versione
            ...
            */
            ...
            // Valorizzazione Triennio di Riferimento dal parametro passato dall'utente
            if (triennioRiferimento != null && !triennioRiferimento.isBlank()) {
    ...
    @Override
    @Transactional(readOnly = true)
    public  List <PiaoDTO> findByCodPafkAndDenominazioneAndVersione(String codPAFK, String denominazione, String versione) {
        try {
            // Validazione
            if (codPAFK == null || codPAFK.isBlank()) {
                throw new IllegalArgumentException("codPAFK è obbligatorio");
            ...
            log.info("Ricerca PIAO per codPAFK={}, denominazione='{}', versione={}",
            ...
            if (piaoList.isEmpty()) {
                log.warn("Nessun PIAO trovato per codPAFK={}, denominazione={}, versione={}",
                    codPAFK, denominazione, versione);
                return List.of();
            }
    ...
    public List<DocumentoPiaoExternalPPDTO> findAllPiaoPubblicati(Long idPiao, String denominazione, String codePa) {
        try {
            ...
            return piaoList.stream().map(piao -> {
                List<AllegatoDTO> allegati = allegatoService.findByIdPiao(piao.getId());
