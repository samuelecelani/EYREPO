...
public class PiaoController {
    ...
    public ResponseEntity<List<PiaoDTO>> findByCodPaFkAndIsCurrent(
            @RequestParam String codPAFK,
            @RequestParam(required = false, defaultValue = "false") boolean isCurrent) {
        return ResponseEntity.ok(piaoService.findByCodPaFkAndIsCurrent(codPAFK, isCurrent));
    }
    ...
    public ResponseEntity<List<PiaoDTO>> consultazionePiao(@RequestParam String codPAFK,
        ...
        List<PiaoDTO> result= piaoService.findByCodPafkAndDenominazioneAndVersione(codPAFK, denominazione, versione, tipologia);
        return ResponseEntity.ok(result);
    }
