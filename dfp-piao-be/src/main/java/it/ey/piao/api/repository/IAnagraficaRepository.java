...
public interface IAnagraficaRepository extends BaseRepository<Anagrafica,Long> {
    ...
    @Query("SELECT a FROM Anagrafica a WHERE a.idSezione1.id = :idSezione1")
    Anagrafica findByIdSezione1(@Param("idSezione1") Long idSezione1);

    @Query(value = """
        SELECT a.* FROM anagrafica a
        WHERE a.X_ACTIVE = true
          AND (:codiceIpa IS NULL OR a.codiceIpa = :codiceIpa)
