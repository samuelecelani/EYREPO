...
public interface IDichiarazioneScadenzaRepository extends BaseRepository<DichiarazioneScadenza, Long>
{
    ...
    @Query("""
            ...
            FROM Piao p
            LEFT JOIN Sezione1 s1 ON s1.piao.id = p.id
            LEFT JOIN Anagrafica a ON a.idSezione1.id = s1.id
     ...
     *   CREATE INDEX IF NOT EXISTS idx_anagrafica_idsezione1  ON anagrafica(idSezione1);
     * </pre>
     */
    @Query(value = """
            ...
            WHERE LOWER(p.denominazione) LIKE LOWER(CONCAT('%', :denominazione, '%'))
              ...
              AND (:tipologiaIstat IS NULL OR EXISTS (
                    SELECT 1 FROM Sezione1 s1
                    JOIN Anagrafica a ON a.idSezione1.id = s1.id
                    WHERE s1.piao.id = p.id
