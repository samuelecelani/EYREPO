...
public class OVPServiceImpl implements IOVPService {
    ...
    public OVPServiceImpl(OVPRepository ovpRepository, OVPMapper ovpMapper,
                          ...
                          StoricoModificaHelper storicoModificaHelper, ITabellaFunzionaleRepository tabellaFunzionaleRepository,
                          IStoricoStatoSezioneRepository storicoStatoSezioneRepository,
                          IOVPPrioritaPoliticaRepository ovpPrioritaPoliticaRepository,
                          IOVPStrategiaRepository ovpStrategiaRepository,
                          IOVPAreaOrganizzativaRepository ovpAreaOrganizzativaRepository,
                          IOVPStakeholderRepository ovpStakeholderRepository,
                          IOVPRisorsaFinanziariaRepository iovpRisorsaFinanziariaRepository) {
    ...
    public void saveOrUpdate(OVPDTO request) {
        ...
        try {
            ...
            if (request.getCampiModificati() != null && !request.getCampiModificati().isBlank() && request.getIdPiao() != null ) {
                storicoModificaHelper.salvaStoricoSePresente(request, request.getSezione21Id(), request.getIdPiao(), Sezione.SEZIONE_21);
            }
            if (request.getStatoSezione() != null && !request.getStatoSezione().isBlank() && !StatoEnum.fromDescrizione(request.getStatoSezione()).name().equals(StoricoStatoSezioneUtils.getStato(storicoStatoSezioneRepository.findByIdEntitaAndCodTipologia(savedEntity.getSezione21().getId(),Sezione.SEZIONE_21.name())))) {
                storicoStatoSezioneRepository.save(
                    StoricoStatoSezione.builder().statoSezione(
                            StatoSezione.builder()
                                ...
                                .build())
                        .idEntitaFK(savedEntity.getSezione21().getId())
    ...
    public void deleteById(Long id, String campiModificati, Long idPiao, String testoSezione, String updatedByNameSurname, String updatedByRole, boolean forceDelete, String statoSezione) {
        ...
        try {
            log.info("Eliminazione OVP con ID={} (forceDelete={})", id, forceDelete);

            OVP ovp = ovpRepository.findById(id)
                .orElseThrow(() ->
            ...
            OVPDTO ovpDto = ovpMapper.toDto(ovp, new CycleAvoidingMappingContext());


                deleteDependencyService.validateBeforeDeleteOrUpdate(id, ovpDto, forceDelete);

                // ERROR
                if (ovpDto.getTypeEnum() == TypeErrorEnum.ERROR) {
                    throw new BusinessException(
                        ovpDto.getErrorCode(),
                        ovpDto.getMessageError(),
                        TypeErrorEnum.ERROR
                    );
                }

                // WARNING
                if (ovpDto.getTypeEnum() == TypeErrorEnum.WARNING) {
                    throw new BusinessException(
                        ovpDto.getErrorCode(),
                        ovpDto.getMessageError(),
            ...
            //  Soft delete  della tabella associativa OVPPrioritaPolitica
            ovpPrioritaPoliticaRepository.softDeleteByOvpId(id, deactivationTime);

            //  Soft delete  della tabella associativa OVPRisorsaFinanziaria
            iovpRisorsaFinanziariaRepository.softDeleteByOvpId(id,deactivationTime);

            //  Soft delete  della tabella associativa OVPAreaOrganizzativa
            ovpAreaOrganizzativaRepository.softDeleteByOvpId(id, deactivationTime);

            // Soft delete  OVPStrategia
            ovpStrategiaRepository.softDeleteByOvpId(id, deactivationTime);

            //  Soft delete  della tabella associativa OVPStakeholder
            ovpStakeholderRepository.softDeleteByOvpId(id, deactivationTime);


            //  Soft delete OVP
            ovpRepository.softDeleteById(id, deactivationTime);

            log.info("OVP con ID={} eliminato con successo", id);


            if (campiModificati != null &&
                !campiModificati.isBlank() &&
                idPiao != null &&
                ovp.getSezione21() != null &&
                ovp.getSezione21().getId() != null) {
            ...
            if (statoSezione != null && !statoSezione.isBlank() &&
                ovp.getSezione21() != null && ovp.getSezione21().getId() != null) {
                if (!StatoEnum.fromDescrizione(statoSezione).name().equals(StoricoStatoSezioneUtils.getStato(storicoStatoSezioneRepository.findByIdEntitaAndCodTipologia(ovp.getSezione21().getId(), Sezione.SEZIONE_21.name())))) {
                    storicoStatoSezioneRepository.save(
                        StoricoStatoSezione.builder().statoSezione(
                                StatoSezione.builder()
                                    ...
                                    .build())
                            .idEntitaFK(ovp.getSezione21().getId())
                            .codTipologiaFK(Sezione.SEZIONE_21.name())
                            .testo(StatoEnum.fromDescrizione(statoSezione).getDescrizione())
            ...
            eventPublisher.publishEvent(new TransactionSuccessEvent<>(ovp));
            ...
            log.error("Errore inatteso in delete (OVP) id={}: {}", id, ex.getMessage(), ex);

            throw new BusinessException(
                BusinessException.INTERNAL_ERROR,
                "Errore interno durante l'eliminazione dell'OVP",
                TypeErrorEnum.ERROR
            );
        }
    }
