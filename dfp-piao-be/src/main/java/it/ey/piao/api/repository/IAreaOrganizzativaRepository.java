...
public interface IAreaOrganizzativaRepository extends BaseRepository<AreaOrganizzativa, Long> {
    ...
    /**
     * Verifica se esistono OVP collegati a un'area organizzativa.
     */
    @Query("SELECT COUNT(oao) > 0 FROM OVPAreaOrganizzativa oao WHERE oao.areaOrganizzativa.id = :idArea")
    boolean existsOvpDependencyByAreaOrganizzativaId(@Param("idArea") Long idArea);

    @Modifying
    @Query("UPDATE AreaOrganizzativa a SET a.active = false, a.deactivationTime = :deactivationTime WHERE a.id = :id")
