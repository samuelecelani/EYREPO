...
public class AmministrazioneServiceImpl implements AmministrazioneService {
    ...
    public void syncAll(List<AmministrazioneDTO> list) {
        ...
        if (codiciRicevuti.isEmpty()) {
            log.warn("Lista amministrazioni vuota: eliminazione di tutte le amministrazioni");
            repository.deleteAll();
        } else {
            repository.deleteByCodiceIpaNotIn(codiciRicevuti);
            log.info("Eliminate amministrazioni non più presenti nella lista esterna");
        }
    }
}
