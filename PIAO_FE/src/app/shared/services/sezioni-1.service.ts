...
export class Sezione1Service {
  ...
  getItemMatrice(idSezione21: number, idSezione1: number): Observable<ItemMatriceDTO[]> {
    return this.http
      .get<
        GenericResponse<ItemMatriceDTO[]>
      >(Path.url(`/ovp/matrice/sezione21/${idSezione21}/sezione1/${idSezione1}`))
      .pipe(map((res) => res.data));
  }
}
