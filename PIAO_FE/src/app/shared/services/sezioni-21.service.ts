...
export class Sezione21Service {
  ...
  save(sezione: Sezione21DTO) {
    return this.http
      .post<GenericResponse<Sezione21DTO>>(Path.url('/sezione21/save'), sezione)
  ...
  getSezione21(piaoDTO: PIAODTO) {
    return this.http
      .post<GenericResponse<Sezione21DTO>>(Path.url(`/sezione21/piao`), piaoDTO)
      .pipe(map((res) => res.data));
  }
  ...
  reloadSezione21AndUpdateSession(): Observable<Sezione21DTO | undefined> {
    ...
    if (piaoDTO) {
      return this.getSezione21(piaoDTO).pipe(
        tap((sezione21) => {
          piaoDTO.sezione21 = sezione21;
