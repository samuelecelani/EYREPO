...
export class PrioritaPoliticaService {
  ...
  save(prioritaPolitica: PrioritaPoliticaDTO): Observable<PrioritaPoliticaDTO> {
    return this.http
      .post<GenericResponse<PrioritaPoliticaDTO>>(Path.url('/priorita-politiche/save'), prioritaPolitica)
      .pipe(
        map((res) => res.data),
        switchMap((savedPrioritaPolitica) =>
          this.reloadAndUpdateSession().pipe(
            map(() => savedPrioritaPolitica)
          )
        )
      );
  }
  ...
  getBySezione(idSezione1: number): Observable<PrioritaPoliticaDTO[]> {
    return this.http
      .get<GenericResponse<PrioritaPoliticaDTO[]>>(Path.url(`/priorita-politiche/sezione/${idSezione1}`))
    ...
    if (piaoDTO && piaoDTO.sezione1?.id) {
      return this.getBySezione(piaoDTO.sezione1.id).pipe(
        tap((prioritaPolitiche) => {
          if (piaoDTO.sezione1) {
            piaoDTO.sezione1.prioritaPolitiche = prioritaPolitiche;
          }
