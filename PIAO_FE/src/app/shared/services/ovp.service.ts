...
export class OvpService {
  ...
  save(ovp: OVPDTO) {
    return this.http
      .post<GenericResponse<OVPDTO>>(Path.url('/ovp/save'), ovp)
      .pipe(map((res) => res.data), switchMap(() => this.sezione21Service.reloadSezione21AndUpdateSession()));
  }
  ...
  delete(id: number) {
    return this.http
      .delete<GenericResponse<void>>(Path.url(`/ovp/${id}`))
      .pipe(map((res) => res.data), switchMap(() => this.sezione21Service.reloadSezione21AndUpdateSession()));
  }
