...
export class NotificaService {
  ...
  getNotifiche(request: IGetNotificheRequest): Observable<IAllNotificationsResponse> {
    return this.http.get<IAllNotificationsResponse>(Path.url('/notification/subscribe'), {
      params: { ...request },
    });
  }
    ...
    fetchEventSource(endpoint, {
      ...
      onmessage: (event) => {
        try {
          ...
          //console.log('EventSource ricevuto:', data);

          // Verifica se è un array diretto o un oggetto con risultati
          ...
          if (Array.isArray(data)) {
            ...
            notifications = data;
            //console.log('Array diretto di notifiche:', notifications.length);
            // Sostituisce l'intera lista preservando lo stato read locale e ordinando per data
            this.notificationsList.set(
              this.sortByCreationDate(notifications.map(preserveLocalReadState))
            );
          } else if (data?.risultati) {
            // Se la risposta ha il campo risultati
            notifications = data.risultati;
            //console.log('Campo risultati:', notifications.length);
            // Sostituisce l'intera lista preservando lo stato read locale e ordinando per data
            this.notificationsList.set(
              this.sortByCreationDate(notifications.map(preserveLocalReadState))
            );
            // Aggiorna il conteggio totale se presente
            if (data?.totaleElementi !== undefined) {
              this.totalNotificationsCount.set(data.totaleElementi);
            }
          } else if (data?.data) {
            // Se la risposta ha il campo data (come per i test)
            notifications = data.data;
            //console.log('Campo data:', notifications.length);
            // Sostituisce l'intera lista preservando lo stato read locale e ordinando per data
            this.notificationsList.set(
              this.sortByCreationDate(notifications.map(preserveLocalReadState))
            );
          } else if (data?.id && data?.message) {
            // È una singola notifica
            //console.log('Singola notifica ricevuta, ID:', data.id);
            const notification = data as INotification;

            // Preserva lo stato read locale
            const finalNotification = preserveLocalReadState(notification);

            // Aggiunge la notifica alla lista solo se non esiste già
            this.notificationsList.update((currentList) => {
              const exists = currentList.some((n) => n.id === finalNotification.id);
              if (exists) {
                //console.log('Notifica già presente, aggiorno:', finalNotification.id);
                // Aggiorna la notifica esistente e riordina
                const updated = currentList.map((n) =>
                  n.id === finalNotification.id ? finalNotification : n
                );
                return this.sortByCreationDate(updated);
              } else {
                //console.log('Nuova notifica aggiunta:', finalNotification.id);
                // Aggiunge e riordina per data
                const newList = this.sortByCreationDate([finalNotification, ...currentList]);
                // Limita a maxNotificationNumber
                return newList.slice(0, this.maxNotificationNumber);
              }
            });
          } else {
            console.warn('Struttura risposta non riconosciuta:', Object.keys(data));
          }

          //console.log('Lista notifiche aggiornata:', this.notificationsList().length);
        } catch (error) {
