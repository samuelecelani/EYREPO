...
export class Sezione22Service {
  ...
  save(sezione: Sezione22DTO) {
    return this.http
      .post<GenericResponse<Sezione22DTO>>(Path.url('/sezione22/save'), sezione)
  ...
  getSezione22(piaoDTO: PIAODTO) {
    return this.http
      .post<GenericResponse<Sezione22DTO>>(Path.url(`/sezione22/piao`), piaoDTO)
      .pipe(map((res) => res.data));
  }
  ...
  reloadSezione22AndUpdateSession(): Observable<Sezione22DTO | undefined> {
    ...
    if (piaoDTO?.id) {
      return this.getSezione22(piaoDTO).pipe(
        tap((sezione22) => {
          console.log('[Sezione22Service] Sezione22 reloaded, notifying subscribers', sezione22);
          piaoDTO.sezione22 = sezione22;
