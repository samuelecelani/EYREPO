...
export function createFormMongoFromPiaoSession<T>(
  ...
  if ((item?.[keys[0]] === undefined || item?.[keys[0]] === null) && !firstValue) {
    return fb.group({
      [keys[0]]: [item?.[keys[0]], [Validators.maxLength(maxSize1), Validators.pattern(pattern)]],
