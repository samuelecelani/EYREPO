...
export class MisureGeneraliComponent implements OnInit {
  ...
  // Dropdown options
  obiettiviPrevenzioneOptions: LabelValue[] = [];
  ...
  ngOnInit(): void {
    ...
    this.loadObiettiviPrevenzioneOptions();
    this.loadStakeholderOptions();
    console.log('[MisureGenerali] ngOnInit, controls length:', this.misureGenerali.length);
  }

  private loadObiettiviPrevenzioneOptions(): void {
    // Carica gli obiettivi di prevenzione dal form parent (sezione23)
    const parentForm = this.formGroup.parent;
    if (parentForm) {
      const obiettiviFormArray = parentForm.get('obiettivoPrevenzione') as FormArray;
      if (obiettiviFormArray) {
        this.obiettiviPrevenzioneOptions = obiettiviFormArray.controls
          .filter((control) => control.get('id')?.value)
          .map((control, index) => ({
            label: control.get('denominazione')?.value || `Obiettivo ${index + 1}`,
  ...
  setCodice(misura: FormGroup, index: number): void {
  ...
  handleRemoveMisura(index: number): void {
    ...
    if (misuraId) {
      this.misurePrevenzioneService.delete(misuraId).subscribe({
        ...
        error: () => {
          this.toastService.error("Errore durante l'eliminazione della misura");
        },
      });
    } else {
  ...
  get misureGenerali(): FormArray {
    return this.formGroup?.get('misuraPrevenzione') as FormArray;
  }
