...
export class ValutazioneRischioComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  handleAttivitaSensibileChange(index: number): void {
      ...
      .subscribe({
        ...
        error: () => {
          this.toastService.error(
            "Errore nel caricamento degli eventi rischiosi per l'attività sensibile selezionata"
          );
        },
      });
  }
  ...
  handleConfirmModalEventiRischiosi(): void {
    if (this.child && this.child.formGroup.valid) {
      ...
      this.eventoRischioService.save(data).subscribe({
        ...
        error: () => {
          this.toastService.error("Errore durante il salvataggio dell'evento rischioso");
        },
      });
  ...
  handleAddValutazione(): void {
    if (this.attivitaSensibileOptions.length === 0) {
      this.toastService.warning(
    ...
    return [
      {
        label: 'Modifica',
        callback: () => {
          this.handleEditEventoRischioso(eventoRischioso, valutazioneIndex);
  ...
  handleEditEventoRischioso(evento: EventoRischiosoDTO, valutazioneIndex: number): void {
    this.currentValutazioneRischioIndex = valutazioneIndex;
  ...
  get valutazioneRischio(): any {
    ...
    if (this.isSortedManuallyValutazione) {
      return formArray;
    }
