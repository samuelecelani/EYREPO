...
export class IndicatoriComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      ...
      next: ({ dimensioni, targets, indicatoriSelect }) => {
    ...
    this.indicatoreService
      ...
      .subscribe({
        next: (data: any) => {
          ...
          // Ricarica la lista indicatori per aggiornare la dropdown
          this.refreshIndicatoriFromServer();
        },
        error: (err) => {
  ...
  handleRemove(element: IIndicatoreWrapper) {
    ...
    // Filtra solo localmente senza ricaricare dal backend
    this.filterIndicatoriDropdown();
  }
  ...
  refreshIndicatoriFromServer(): void {
    // Ricarica gli indicatori dal servizio (usato solo all'apertura del modal)
    this.indicatoreService
  ...
  filterIndicatoriDropdown(): void {
    // Filtra solo localmente senza ricaricare dal backend (ottimizzato per prestazioni)
    ...
    console.log('Indicatori dropdown filtrati:', this.indicatoriDropdown);
  }
