...
export class MatriceComponent implements OnInit {
  ...
  loadData(): void {
    if (!this.idSezione21 || !this.idSezione1) {
      console.warn('idSezioni non definito, skip caricamento matrice');
      return;
    }

    this.sezione1Service.getItemMatrice(this.idSezione21, this.idSezione1).subscribe((data) => {
      this.matrice = data;
