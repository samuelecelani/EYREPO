...
export class Sezione1Component extends BaseComponent implements OnInit, ISezioneBase {
  ...
  ngOnInit(): void {
    ...
    // Crea il form interno
    this.createForm();
  }

  createForm() {
    const sezione1 = this.sezione1Data || this.piaoDTO?.sezione1 || new Sezione1DTO();
  ...
  prepareDataForSave(): Sezione1DTO {
    let sezione1 = this.sezione1Data || this.piaoDTO?.sezione1 || new Sezione1DTO();
  ...
  validate(): Observable<any> {
    const sezione1Id = this.sezione1Data?.id || this.piaoDTO?.sezione1?.id || -1;
