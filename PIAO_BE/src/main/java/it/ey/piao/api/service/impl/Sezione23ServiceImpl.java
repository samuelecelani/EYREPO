...
public class Sezione23ServiceImpl implements ISezione23Service {
    ...
    public Sezione23DTO getOrCreateSezione23(PiaoDTO piao) {
        ...
        try {
            ...
            Sezione23 existing = sezione23Repository.findByPiao(piaoMapper.toEntity(piao, context));
            ...
            if (existing != null) {
                ...
                if (response.getObiettivoPrevenzione() != null && !response.getObiettivoPrevenzione().isEmpty()) {
                    response.getObiettivoPrevenzione().forEach(obiettivoDTO -> {
                ...
                if (response.getObblighiLegge() != null && !response.getObblighiLegge().isEmpty()) {
                    response.getObblighiLegge().forEach(obbligoLeggeService::loadMongoDataForObbligo);
                }
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED)
    public Sezione23DTO saveOrUpdate(Sezione23DTO request) {
        if (request == null) {
            throw new IllegalArgumentException("La richiesta non può essere nulla");
        }

        CycleAvoidingMappingContext context = new CycleAvoidingMappingContext();
        Sezione23DTO response;

        try {
            Sezione23 entity = sezione23Mapper.toEntity(request, context);
            // Salvo lo stato dell'oggetto per un eventuale rollback
            if (entity.getId() != null) {
                sezione23Repository.findById(entity.getId()).ifPresent(existing ->
                    eventPublisher.publishEvent(new BeforeUpdateEvent<>(Sezione23.class, existing))
                );
            }
            ...
            response = sezione23Mapper.toDto(savedEntity, context);
            ...
            saveMongoDataSezione23(response, request);

        } catch (Exception e) {
            log.error("Errore durante saveOrUpdate Sezione23 per id={}: {}",
    ...
    @Override
    @Transactional(readOnly = true)
    public Sezione23DTO findByPiao(PiaoDTO piao) {
        if (piao == null || piao.getId() == null) {
            throw new IllegalArgumentException("Il PIAO non può essere nullo e deve avere un ID valido.");
        }
        ...
        try {
            ...
            log.error("Errore durante findByPiao Sezione23 per PIAO id={}: {}", piao.getId(), e.getMessage(), e);
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED)
    public void saveMongoDataSezione23(Sezione23DTO response, Sezione23DTO request) {
        ...
        try {
            ...
            log.debug("Dati MongoDB salvati per Sezione23 id={}", response.getId());
        } catch (Exception e) {
            log.error("Errore salvataggio dati MongoDB per Sezione23 id={}: {}", response.getId(), e.getMessage(), e);
