...
public class Sezione1ServiceImpl implements ISezione1Service {
    ...
    public Sezione1DTO getOrCreateSezione1(PiaoDTO piao) {
        ...
        try {
            ...
            Sezione1 existing = sezione1Repository.findByPiao(piaoMapper.toEntity(piao, context));
            ...
            if (existing != null) {
                ...
                // Carica gli StakeHolders dal PIAO
                Optional<Piao> piaoEntity = piaoRepository.findById(piao.getId());
                if (piaoEntity.isPresent()) {
                    List<StakeHolder> stakeHolders = stakeHolderRepository.findByPiao(piaoEntity.get());
                    if (stakeHolders != null && !stakeHolders.isEmpty()) {
                        response.setStakeHolders(
                            stakeHolders.stream()
                                .map(sh -> stakeHolderMapper.toDto(sh, context))
                                .toList()
                        );
                    }
                }
    ...
    @Override
    @Transactional(readOnly = true)
    public Sezione1DTO findByPiao(PiaoDTO piao) {
        if (piao == null || piao.getId() == null) {
            throw new IllegalArgumentException("Il PIAO non può essere nullo e deve avere un ID valido.");
        }
        ...
        try {
            ...
            log.error("Errore durante findByPiao Sezione1 per PIAO id={}: {}", piao.getId(), e.getMessage(), e);
    ...
    public Sezione1DTO saveOrUpdate(Sezione1DTO request) {
        ...
        try {
         ...
         //   SezioneUtils.sanitizeJoinChildren(entity);
            ...
            response = sezione1Mapper.toDto(savedEntity, context);
            ...
            if (request.getStakeHolders() != null && !request.getStakeHolders().isEmpty()) {
                ...
                Optional<Piao> piao = piaoRepository.findById(request.getIdPiao());
                ...
                List<StakeHolder> stakeHolders = request.getStakeHolders().stream()
                    .map(stakeHolderDTO -> {
                        ...
                        stakeHolder.setPiao(piao.orElse(null));
                        return stakeHolder;
                    }
                    )
                    .toList();

                response.setStakeHolders(stakeHolderRepository.saveAll(stakeHolders).stream()
                    .map(e -> stakeHolderMapper.toDto(e, context))
                    .toList());
            } else {
                response.setStakeHolders(Collections.emptyList());
            }
            ...
            saveMongoDataSezione1(response, request);
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED)
    public void saveMongoDataSezione1(Sezione1DTO response, Sezione1DTO request) {
        ...
        try {
            ...
            // Salva Social MongoDB (prende da request, setta su response)
            response.setSocial(
                Optional.ofNullable(request.getSocial())
                    ...
                    .map(saved -> commonMapper.socialEntityToDto(saved, context))
                    .orElse(null)
            );

            log.debug("Dati MongoDB salvati per Sezione1 id={}", response.getId());
        } catch (Exception e) {
            log.error("Errore salvataggio dati MongoDB per Sezione1 id={}: {}", response.getId(), e.getMessage(), e);
