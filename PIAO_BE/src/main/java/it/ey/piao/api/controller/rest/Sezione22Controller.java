...
public class Sezione22Controller {
    ...
    @PostMapping("/save")
    public ResponseEntity<Sezione22DTO> save(@RequestBody Sezione22DTO request) {
        return ResponseEntity.ok(sezione22Service.saveOrUpdate(request));
    }
    ...
    public ResponseEntity<Sezione22DTO> getByIdPiao(@PathVariable Long idPiao) {
        Sezione22DTO sezione22 = sezione22Service.findByIdPiao(idPiao);
        if (sezione22 == null) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(sezione22);
    }
}
