...
public class Sezione22ServiceImpl implements ISezione22Service {
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED)
    public Sezione22DTO saveOrUpdate(Sezione22DTO request) {
        if (request == null) {
            throw new IllegalArgumentException("La richiesta non può essere nulla");
        }

        CycleAvoidingMappingContext context = new CycleAvoidingMappingContext();
        Sezione22DTO response;
        ...
        try {
            ...
            response = sezione22Mapper.toDto(savedEntity, context);


            if( request.getObbiettiviPerformance() != null && !request.getObbiettiviPerformance().isEmpty()) {
                response.setObbiettiviPerformance(obbiettivoPerformanceService.saveOrUpdateAll(request.getObbiettiviPerformance()));
