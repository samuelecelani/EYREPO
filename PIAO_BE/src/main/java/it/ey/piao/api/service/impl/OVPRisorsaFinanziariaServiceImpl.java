...
public class OVPRisorsaFinanziariaServiceImpl implements IOVPRisorsaFinanziariaService {
    ...
    @Override
    public List<OVPRisorsaFinanziariaDTO> saveOrUpdate(List<OVPRisorsaFinanziariaDTO> request) {
        try {
            // Raggruppo per idOvp per caricare ogni OVP una sola volta
            Map<Long, OVP> ovpCache = new HashMap<>();

            // Mappo i DTO in entità
            List<OVPRisorsaFinanziaria> entities = request.stream()
                .map(dto -> {
                    // Carico l'OVP solo se non è già nella cache
                    OVP ovp = ovpCache.computeIfAbsent(dto.getIdOvp(),
                        ovpRepository::getReferenceById);
            ...
            // Salvo le entità
            List<OVPRisorsaFinanziaria> response = iovpRisorsaFinanziariaRepository.saveAll(entities);

            // Mappo le entità salvate in DTO
            return ovpRisorsaFinanziariaMapper.toDtoList(response);
